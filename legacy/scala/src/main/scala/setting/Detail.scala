...
sealed trait DownloadValidation

object DownloadValidation {
  case object Valid extends DownloadValidation
  final case class NoData(reason: String) extends DownloadValidation
  final case class Invalid(reason: String) extends DownloadValidation
}
...
abstract class Detail(val firstDate: LocalDate, _strDateOption: Option[LocalDate], _endDate: LocalDate) {
   ...
   *   - Valid: keep and read normally.
   *   - NoData: keep as a 0-byte sentinel so non-trading days are not retried.
   *   - Invalid: delete and retry, because this may be a provider/runtime failure.
   */
  def validate(downloaded: java.io.File): DownloadValidation = DownloadValidation.Valid
  ...
  protected def validateCSVHeaderDate(downloaded: java.io.File,
                                       ...
                                       minDataRows: Int,
                                       searchLines: Int = 5,
                                       encoding: String = "Big5-HKSCS"): DownloadValidation = {
    if (!downloaded.exists()) return DownloadValidation.Invalid("file is missing")
    if (downloaded.length() == 0L) return DownloadValidation.Valid
    val source = scala.io.Source.fromFile(downloaded, encoding)
    try {
      val lines = source.getLines().toList
      if (lines.isEmpty) return DownloadValidation.Invalid("file has no lines")
      if (lines.headOption.exists(_.toLowerCase.contains("<html>"))) return DownloadValidation.Invalid("HTML error page")
      ...
      if (!headerMatch) {
        ...
        return DownloadValidation.Invalid(s"header date mismatch: expected marker '$expectedDateMarker' in first $searchLines lines; got head='$headPreview'")
      }
      val dataRows = lines.drop(2).count(l => l.trim.nonEmpty)
      if (dataRows < minDataRows) return DownloadValidation.Invalid(s"only $dataRows data rows (expected >= $minDataRows)")
      DownloadValidation.Valid
    } finally {
      source.close()
    }
  }
  ...
  protected def validateCSVSchema(downloaded: java.io.File,
                                   ...
                                   encoding: String = "Big5-HKSCS",
                                   allowHeaderOnlyNoData: Boolean = false): DownloadValidation = {
    if (!downloaded.exists()) return DownloadValidation.Invalid("file is missing")
    ...
    try {
      ...
      if (missingKeywords.nonEmpty) return DownloadValidation.Invalid(s"missing header keywords: ${missingKeywords.mkString(", ")}")
        ...
        lines.find(_.contains(headerMarker)) match {
          case Some(headerLine) =>
            ...
            if (columns < minHeaderColumns)
              return DownloadValidation.Invalid(s"header has only $columns columns (expected >= $minHeaderColumns) — likely legacy/partial format")
          case None =>
        }
      }
      val dataRowPattern = """^(="?\d[^,]*"?|"?\d[^,]*"?),.*""".r
      val dataRows = lines.count(l => dataRowPattern.findFirstIn(l).isDefined)
      if (allowHeaderOnlyNoData && dataRows == 0 && lines.exists(_.contains("說明")))
        return DownloadValidation.NoData("header-only no-data response")
      if (dataRows < minDataRows) return DownloadValidation.Invalid(s"only $dataRows data rows (expected >= $minDataRows)")
      DownloadValidation.Valid
    } finally {
      source.close()
  ...
  def getYearsOfExistFiles: Set[Int] = {
    val fileNamePattern = """(\d+)_.*\.csv""".r
    files.flatMap(file => fileNamePattern.findFirstMatchIn(file.name).map(_.group(1).toInt)).toSet
  }

  def getTuplesOfExistFiles: Set[(Int, Int)] = {
    val fileNamePattern = """(\d+)_(\d+).*""".r
    files.flatMap { file =>
      fileNamePattern.findFirstMatchIn(file.name).map { m =>
