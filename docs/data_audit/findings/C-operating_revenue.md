...
**影響**:Scala strategy 層與所有走 `src/quantlib/db.py` pg-attach 模式的腳本,
在最近三個月看不到半個金控的營收。
...
- `src/quantlib/crawl/sources/operating_revenue.py::refresh` 每次執行都重抓**最近 3 個月**
  (`_REFRESH_MONTHS = 3`),經 `src/quantlib/crawl/sink.py::Sink.upsert`(刪整個
...
營收從 cache 消失。而 `src/quantlib/serenity/daily.py:242`(live 每日 loop)正是會呼叫它的地方。

**這代表回測不可重現**:同一份程式、同一個 cache 檔名,結果取決於今天早上先跑了哪一條。
`operating_revenue` 是 S / Serenity 進場訊號的原料(`src/quantlib/serenity/daily.py:135`
讀 `monthly_revenue_yoy`、`src/quantlib/apex/data.py:223`),屬 money path。
...
`src/quantlib/crawl/sources/operating_revenue.py:36`
...
3. **2013 年前 `individual` 與 `consolidated` 併存**:27,917 組 (年,月,代號) 同時
   ...
   `src/quantlib/apex/data.py:218-228` 有正確去重(優先 consolidated);
   `src/quantlib/serenity/daily.py:135` 沒篩,但只讀近月故無實害。
...
```
$ uv run --project . python src/quantlib/audits/05_revenue_audit.py
_duckdb.BinderException: Binder Error: Catalog "pg" does not exist!
```

`src/quantlib/db.py::connect()` 預設 `use_cache=True`,cache 存在就回 cache 連線;
...
6. **修好 `src/quantlib/audits/05_revenue_audit.py`**:改用 `connect(use_cache=False)`
