...
**同類掃描**:Python 側的替代品 `research/strat_lab/raw_quarterly.py:127-146, 176-179` 用
...
| Python 研究層 | 明文棄用 | `research/strat_lab/raw_quarterly.py:10`:「We DELIBERATELY do NOT use … financial_index_quarterly (VIEW with margin computation we can't fully verify)」 |
...
`research/strat_lab/raw_quarterly.py:127-146, 176-179` 的
...
| 2 | `materialized_view/5_…_individual.sql`、`3_cash_flows_individual.sql`、`research/strat_lab/raw_quarterly.py` | 單季化加「上一筆必須是相鄰季」守衛,缺季給 NULL |
