...
- `research/strat_lab/iter_52_ownership_flow_alpha.py:87` — `WHERE date BETWEEN START AND END`,
  **沒有 market 篩選**,而 `START = 2005-01-03`(`research/strat_lab/iter_40_research_campaign.py:57`)
  → 直接吃進 31.9 萬列假資料。
- `research/futures/strategies.py:183-193` — `SUM(foreign_held_shares)/SUM(outstanding_shares) GROUP BY date`,
...
- Serenity(`research/serenity/engine.py:1000`)與 Evergreen(`ev48_chip_axes.py:46`、`ev18_make_packs.py:96`)
...
- **schema 無型別降級**:PG 11 欄 → cache 9 欄,只少了 `id`(代理鍵)與 `company_name`;
  `research/db.py:149-153` 的 pg-attach 對照 view 欄位與順序和 cache 表逐字相同,parity 沒破。
...
```bash
uv run --project research python docs/data_audit/scripts/C-foreign_holding_ratio/01_counts.py
uv run --project research python docs/data_audit/scripts/C-foreign_holding_ratio/02_value_parity.py       # ~28s
uv run --project research python docs/data_audit/scripts/C-foreign_holding_ratio/03_coverage_gaps.py
uv run --project research python docs/data_audit/scripts/C-foreign_holding_ratio/04_stale_repeat_scan.py
uv run --project research python docs/data_audit/scripts/C-foreign_holding_ratio/05_filename_vs_content_date.py
uv run --project research python docs/data_audit/scripts/C-foreign_holding_ratio/06_value_anomaly.py
uv run --project research python docs/data_audit/scripts/C-foreign_holding_ratio/07_sample_parity.py
uv run --project research python docs/data_audit/scripts/C-foreign_holding_ratio/08_tpex2010_impact.py
uv run --project research python docs/data_audit/scripts/C-foreign_holding_ratio/09_missing_trading_days.py
```
