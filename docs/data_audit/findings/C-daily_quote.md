...
在資料庫裡整列是 0**,cache 與 `src/quantlib/prices.py` 都用 `cash_dividend > 0` 過濾,
...
是靠寫入端自律(`src/quantlib/crawl/sink.py:35-59` 的「刪整日 + 插」),不是靠資料庫。
...
而 `src/quantlib/data_calendar.py:44-57` 的 `is_trading_day()` **就是讀這些 sentinel
當休市日曆**。實測:
...
`src/quantlib/crawl/update.py:30-41` 的 `_missing_days()` 只補 `is_trading_day` 為真的
...
- 舊路徑:Scala `Main update` → 原始 CSV 落 `data/` → 匯入 PG → `research/cache_tables.py`
  砍掉整個 cache 重建。
- 新路徑:`src/quantlib/crawl/update.py` + `src/quantlib/crawl/sources/daily_quote.py`
...
守護(`src/quantlib/crawl/tests/test_parity.py`,對照日 2026-07-17 逐位比對),設計上是
乾淨的。但衍生兩個後遺症:

1. **RAW 封存出現斷點**:`data/daily_quote/twse/2026/` 沒有 `2026_7_20.csv`。
   `src/quantlib/paths.py` 明寫 RAW 是「不可重生的事實地基」,現在交易日的原始 CSV
...
fail-closed → 候選被靜靜砍光。目前 `src/quantlib/tri/daily.py::ensure_fresh_cache` 會
...
- `src/quantlib/prices.py:141-144` — `SELECT date AS ex_date, company_code, cash_dividend FROM ex_right_dividend ... AND cash_dividend > 0`
