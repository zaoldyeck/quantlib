...
**為什麼要緊**:Serenity 引擎(`research/serenity/engine.py:964`)與各回測都用
...
`research/data_calendar.py::is_trading_day`(讀 0-byte 休市 sentinel、能認出颱風假)
也判為交易日。
...
- Python 爬蟲 `research/crawl/sources/stock_per_pbr.py` → **直寫 cache.duckdb**
  (`research/crawl/sink.py`),完全不經過 PG
...
- Python:`research/crawl/sources/stock_per_pbr.py::_parse` 有欄位位移守護
  (`_guard` → `SchemaDrift`),但**沒有任何日期檢查**——同樣的錯位在雲端 VM 上會原封重演。

另外,Python 管線有一個會製造 BUG 2 的機制:`research/crawl/update.py::_refresh_daily`
...
pg-attach 模式的對照 view(`research/db.py:103-106`)欄位與順序和 cache 表完全一致,
...
2. **抓失敗要留疤**:`research/crawl/update.py::_refresh_daily` 對 `fetch_day` 回 None
...
```
執行:`uv run --project research python docs/data_audit/scripts/C-stock_per_pbr/<script>.py`
