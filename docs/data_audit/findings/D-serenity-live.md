...
**範圍**:`src/quantlib/serenity/engine.py`(回測引擎，計分/模擬/退場真源)+
`src/quantlib/trading/exit_replay.py`(live 逐日重放退場檢查)。
連帶讀了兩支的直接依賴以判定計算式正確性:`src/quantlib/serenity/exit_rules.py`
(六道門單一真源)、`src/quantlib/serenity/replay_2025.py`(`load_price_features` /
`score_candidates` / `revenue_report_date`)、`src/quantlib/serenity/daily.py`(live 執行器
...
**指標**:trailing stop / 絕對停損的價格基準 —— `src/quantlib/trading/exit_replay.py:77-82`
(`SELECT ... closing_price FROM daily_quote`)、`:152-159`(`cum_max(closing_price)` +
`peak_floor`);LIVE 同病:`src/quantlib/serenity/daily.py:341,383,389`(`market_data` 取
`closing_price`、`peak=max(stored,px)`、`evaluate_exit(px=raw...)`)。對照的回測引擎
`src/quantlib/serenity/engine.py:189,207,449,635-638` 走 `load_price_features` 的 **還原**
...
`uv run --project . python -c` 對 cache 跑上表 SQL(見稽核 transcript)。
...
**指標**:訊號(thesis)退場門 —— `src/quantlib/serenity/exit_rules.py:44-47`
(第 5 門 `inst20<0 且 px<anchor`、第 6 門 `yoy3<0`)對照 `src/quantlib/serenity/engine.py:592-598`
...
`src/quantlib/serenity/tests/` 只有 `test_execution.py`,**無 parity test 鎖死** live↔engine
...
**指標**:`atr20_pct` —— `src/quantlib/serenity/replay_2025.py:196-199`,經 engine.py
...
**指標**:`peg_target_mult` 動態止盈倍數 —— `src/quantlib/serenity/engine.py:394-400`,
...
- `src/quantlib/serenity/daily.py`(live 執行器,**不在本單位範圍**)的 live book 用
