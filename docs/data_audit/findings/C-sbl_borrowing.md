...
3. **沒有任何守護擋得住這一類問題**:`research/audits/04_cross_verify.py` 這支「檔名日期
   vs 內容日期」的檢查器,程式碼第 49 行寫死只掃 `data/index`,借券表從來沒被掃過;
   `research/audits/03_full_data_audit.py` 也完全沒有借券的檢查。
...
| 兩條投影是否漂移 | `research/cache_tables.py:53`(CREATE TABLE)與 `research/db.py:145-148`(pg-attach view)正規化後**逐字元相同**(程式化比對回 True) |
...
| 守護 | `research/audits/04_cross_verify.py:49` 寫死 `root = Path("data/index")`,借券表不在掃描範圍 |
...
| 檔案 | 用法 | 受影響形態 |
|---|---|---|
| `research/serenity/engine.py:987-997`(**現役策略**) | `shift(20)` 位置位移算 `sbl_chg_20d` | 幽靈列佔掉窗格 → 窗口位移。最後一個汙染日 2022-06-19,對 2330 而言決策日 ≤ **2022-07-15** 的窗仍跨過幽靈列;2022-07-16 之後乾淨 |
| `research/evergreen/ev48_chip_axes.py:37-44` | `shift(5)` | 同上(位置位移) |
| `research/evergreen/ev18_make_packs.py:87-95` | `row_number() OVER (ORDER BY date DESC) LIMIT 20` | 幽靈日進入 DISTINCT date 清單 → 20 日窗變 18~20 個真實交易日 |
| `research/evergreen/make_tables_chips.py:56-63` | 20 日窗 first/last | 同上 |
| `research/apex/data.py:195-198` + `experiments/f10_chip_axes.py:25-31` | `shift(5)` | 同上 |
| `research/strat_lab/iter_52_ownership_flow_alpha.py:100-111` | 日期區間 + 位移 | 同上 |
| `research/futures/strategies.py:174-182` | `SUM(...) GROUP BY date` 全市場加總 | 10 個真交易日的加總是別天的值;幽靈日多出 16 個日期 |
| `research/experiments/sprint_a_signal_prototype.py:84-117` | `LAG` over date | 同上 |
...
4. **立三道守護(先紅後綠)**:
   ...
   * 把 `research/audits/04_cross_verify.py:49` 從寫死 `data/index` 改成掃**所有**
...
* schema 對照:PG `information_schema.columns`(11 欄)vs DuckDB `DESCRIBE`(9 欄);
  `research/cache_tables.py:53` 與 `research/db.py:145-148` 兩條投影字面比對
...
* 缺口三法:證人投票(5 張日頻表)、純日曆(`research.data_calendar.is_trading_day`
...
* 既有守護盤點:`research/audits/03_full_data_audit.py`(無 sbl 檢查)、
  `research/audits/04_cross_verify.py:49`(寫死 `data/index`)、`research/tests/`(無 sbl 測試)
