...
`src/quantlib/prices.py` 誤打誤撞用對了),但**任何拿這欄算「配息收入」「殖利率」「現金股利」的人
...
雲端 Python 爬蟲更是明文分岔(`src/quantlib/crawl/sources/ex_right_dividend.py:8-9`):
...
**還原股價 —— legacy 段誤打誤撞是對的。** `src/quantlib/prices.py:290-293`:
...
**配息收入 / 殖利率 —— 直接錯。** `src/quantlib/experiments/chase_trailing_stop.py:160` 用
...
1. 加 `stock_dividend_ratio`(配股率,無量綱)與 `announce_note` 欄;`cash_dividend` 恢復成
   真正的現金股利(元/股)。
2. `src/quantlib/prices.py` 的因子改成 `(pre − cash_dividend) / pre / (1 + stock_dividend_ratio)`;
...
4. 兩個生產者(Scala reader、`src/quantlib/crawl/sources/ex_right_dividend.py`)寫同一欄卻定義不同
...
- `src/quantlib/strat_lab/raw_quarterly.py:88-91`:「cf has no market split — join by company_code only」
...
| `src/quantlib/apex/data.py:221` | `ORDER BY CASE type WHEN 'consolidated' THEN 0 ELSE 1 END` | ✅ 正確 |
| `src/quantlib/evergreen/make_tables.py:51-53` | `ORDER BY monthly_revenue DESC` | ⚠️ 挑「數字大的」而不是挑合併 |
| `src/quantlib/serenity/daily.py:134-137` | 沒有任何去重 | ⚠️ 同月兩列都會進 `usable[-3:]`,3 個月均值變 1.5 個月 |
...
消費者 `src/quantlib/db.py:142` 只是註冊 view、沒有聚合,所以還沒爆。
