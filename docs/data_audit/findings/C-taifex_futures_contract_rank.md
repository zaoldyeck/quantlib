...
垃圾值**,而這個值**正被 `src/quantlib/futures/strategies.py` 當交易訊號在用**。
...
### 病灶
`src/quantlib/futures/taifex.py:56` 的過濾條件
...
### 消費端(確認會咬人,不是死碼)
`src/quantlib/futures/strategies.py`:
...
**重建 cache** 並回跑 `src/quantlib/futures/tests/test_taifex_futures.py`;另加一條守護:
