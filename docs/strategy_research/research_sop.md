...
2. **資料正確性優先於模型複雜度**
   - 價格必須經 `src/quantlib/prices.py` 產生 total-return-equivalent adjusted OHLCV。
...
```bash
uv run --project . pytest src/quantlib/tests/test_db.py src/quantlib/tests/test_prices.py src/quantlib/tests/test_industry_taxonomy.py -q
```

最低接受條件：

- `quantlib.db.connect()` 可平行 read-only 開啟。
- 還原價格測試通過。
- `industry_taxonomy_pit` 可以 point-in-time as-of join，且不會退回 latest-static 分類。

## 3. 產業分類標準

正式產業分類來源是 `src/quantlib/industry_taxonomy.py` 建出的 `industry_taxonomy_pit`。
...
```bash
uv run --project . python src/quantlib/experiments/serenity_valuation_methods_replay_2025.py --start 2025-01-01
```
...
```bash
uv run --project . python src/quantlib/experiments/serenity_industry_first_replay_2025.py --start 2025-01-01
```

此範例是 SOP 可回測性的第一版，使用 `src/quantlib/experiments/serenity_industry_thesis_registry_2025.csv` 作為 thesis registry。若要升級成可列入正式策略排名的結果，必須把 registry 補成嚴格研究日誌：每筆 thesis 要有可驗證的證據日期、來源連結與當時可得的 analyst note。

## 5. Backtest 實作標準

1. **價格**
   - 必須使用 `src/quantlib/prices.py` 的 adjusted OHLCV。
   - 不得用 raw close 做 NAV。
...
- 自動交易與 broker 操作：`src/quantlib/trading/README.md`。
