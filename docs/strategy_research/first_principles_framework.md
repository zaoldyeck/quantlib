...
## D1 的決定性實驗(2026-07-24;使用者質疑「為何不能用近幾年資料研發?每個 regime 不一樣」)

先前 D1(用全史、不縮窗)的證據只到「edge 跨期 IC 同號」——**那證明 edge 穩定,不證明「近期
refit 一定更差」**。故做真樣本外對決(`strat_lab/window_length_wf.py`):81 格參數(trail×
loser_time×time_stop×slots)× 10 個 OOS 年(2017-2026)× 5 種回看窗長,每年用「前 N 年」
依 Sortino 選參 → 記該參數在**從未參與選擇的那一年**的報酬,對照 canonical 固定參數。

| 回看窗長 | pooled OOS 年化 | canonical | 勝年數 | 平均超額 |
|---|---|---|---|---|
| 1 年 | +64.8% | +81.5% | 2/10 | **−21.6%** |
| 2 年 | +72.8% | +81.5% | 1/10 | −9.1% |
| 3 年 | +73.5% | +81.5% | 2/10 | −9.3% |
| 5 年 | +78.8% | +81.5% | 2/10 | −5.6% |
| 全史(擴張) | +78.3% | +81.5% | 2/10 | −6.2% |

**結論(實驗證據,非論述)**:
1. **窗越短、樣本外越差**(1 年 −21.6pp → 5 年 −5.6pp),單調關係。使用者的直覺「regime 不同」
   **前提正確但推論不成立**:regime 確實各異,但**下一個 regime 的方向無法從剛過去的 regime 推得**;
   用近 1 年最佳參數 = 賭「未來像剛過去」,實測平均每年賠 21.6pp。
2. **連全史 refit 也輸固定參數(−6.2pp)** → **「選參數」這個動作本身就有害**,不只短窗問題。
   canonical(trail .35/loser 15/time 30/slots 5)不是選出來的最佳,是原始設計,反而最穩。
3. train Sortino ↔ OOS 報酬 Pearson **+0.148**(完整 S 參數空間,n=810):微正但幾乎無預測力
   ——「train 期表現好」對「樣本外表現好」的資訊量近乎零。(3.1 在單一動能因子上量到 −0.6~−0.8,
   本實驗在完整策略參數空間上量到 ≈0,兩者共同否定「挑 train 最佳」的做法。)
4. **機制解讀**:S 的參數(trail/time/slots)管的是**風險與節奏**,不是市場觀點;真正 regime 敏感
   的「該不該進場」由**事件驅動的營收訊號自帶適應性**(景氣好時營收加速的標的自然變多)。
   所以 S 不需要、也不該用參數層去追 regime。
5. **例外情境(誠實界定)**:本結論適用於「參數 refit」。若未來出現**結構性斷裂**(制度改變、
   資料源改變、策略容量到頂),那不是 refit 能解決的,而是要重新研發——判準是 edge 本身的
   IC 是否反號/衰減(見 D1 原則),不是近期績效。

**caveat**:切片法評估(路徑相依近似,walk-forward 文獻標準做法);參數空間為四軸 81 格
(未含因子權重——但因子權重 ±20% 已另驗 spread 僅 7.8%,對結論影響有限)。
