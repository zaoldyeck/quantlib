...
## 已拍板的方法論決策(2026-07-24,附證據;`strat_lab/methodology_decisions.py`)

**D1 回測歷史窗 = 全史,不縮窗、不用固定窗優化參數。**
- 證據:三個 edge(營收加速度/近 52 週高/20 日位置)在早期 2014-2019 與近期 2020-2026 的截面
  IC **同號且都極顯著**(rev_accel +0.050→+0.035、high_52w +0.051→+0.055、close_pos +0.035→+0.036,
  t 全 >10)→ edge 結構性、跨 regime 穩。
- 決策:**全史驗證(最大統計力 + regime 覆蓋)+ 不優化參數(3.1:固定窗選參過擬合)+ regime
  一致性當關卡(逐年 OOS 下界)**。問題不是「用過去幾年優化」,是「全史驗證 + 穩健選參」。縮到
  「過去 N 年」反而丟證據 + 踩 3.1 陷阱。(例外:若某 edge 跨期 IC 反號/衰減 = 非結構性,才縮窗
  或棄用——由證據決定,不是預設縮窗。)

**D2 策略品質 KPI = Sortino(取代 Sharpe)+ Calmar + DSR/PBO + 逐年 OOS 下界,不用單一比率。**
- 證據:S 變體 Sortino(3.28)遠高於 Sharpe(2.10)——正偏策略上行波動大,**Sharpe 不合理懲罰
  上行,Sortino 只罰下行波動,更合理**。
- 決策:比較策略好壞用**組合**:① **Sortino**(風險調整,取代 Sharpe)② **Calmar**=CAGR/MDD
  (回撤是實際會痛的)③ **DSR/PBO**(真不真、過不過擬合)④ **逐年 OOS 下界/最差年**(regime 韌性)。
  主目標仍是 OOS 幾何成長率(§5)。禁止只報單一 in-sample 比率就聲稱好。

**D3 回測已極速化。** run_s 單次 4.3s(合 5s 迭代);瓶頸 prep 特徵組裝 31s → `strategy_s.prep_cached`
磁碟快取(key 含 cache.duckdb mtime,資料一變即失效)→ 0.55s(**38x**),迭代回測 ~5s。變體/掃描
campaign 用 prep_cached(一次算、多次秒回)+ 並行 run_s。
