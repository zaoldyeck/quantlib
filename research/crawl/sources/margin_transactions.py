...
- **name-strip 只清數值欄** → company_name 保留原文(只去頭尾空白),不像 Scala 連名稱
  一起 `.replace(" ","").replace(",","")`(本源名稱無內部空白/逗號,parity 已證等值)。
...
def _clean(cell: str) -> str:
    """數值欄清洗,對齊 Scala `.replace(" ","").replace(",","")`。"""
...
def _parse_twse(text: str, day: Date) -> pl.DataFrame | None:
    ...
    for r in rows:
        ...
        recs.append({
            ...
            "company_name": r[1].strip(),         # 只清頭尾,保留名稱原文
