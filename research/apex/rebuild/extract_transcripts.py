...
python 區塊與 Write 內容,按 trials.jsonl 的 trial name 歸檔:
    ...
    research/apex/rebuild/recovered/{seq:03d}_{主要batch}.py   逐字原版代碼
    research/apex/rebuild/recovered/index.json                 trial → 檔案對照
    stdout                                                     coverage 統計

Run: uv run --project research python -m research.apex.rebuild.extract_transcripts
依賴 cache: 否
"""
...
OUT = Path("research/apex/rebuild/recovered")
TRIALS = Path("research/apex/ledger/trials.jsonl")
...
def main() -> None:
    ...
    covered = set(index)
    missing_before = json.load(open("research/apex/rebuild/packs.json"))
