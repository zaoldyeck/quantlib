...
| 頻率 | OOS CAGR | Sharpe | MDD | P5 | 終值 | 換手次數 |
|---|---|---|---|---|---|---|
| 每月 | 79.6% | 1.90 | −36.7% | 0.478 | 286x | 12 |
| 每季 | 87.6% | 2.03 | −37.5% | 0.557 | 536x | 4 |
| 每半年 | 89.7% | 2.07 | −37.5% | 0.575 | 632x | 4 |
| **每年** | **90.7%** | **2.08** | −38.0% | **0.583** | **685x** | 3 |
| 永不(凍結首選) | 76.3% | 1.83 | −39.7% | 0.455 | 316x | 0 |

**三個結論**:(1) **年更 = 最優**,單調 每月<每季<每半年<每年(Sharpe 選同序);
(2) **月更最差(近永不)**:3 年窗排名近似平手每月翻動 → 追雜訊 + 換手,不是抓
regime;(3) **永不也差**:錯過 2020 ax4→ax6 的真 regime 轉變(M01 3 年窗選擇史所示)。
=倒 U:太頻繁追雜訊、不更錯過轉變、**年度剛好**。**現行 2027-07 年度 refit 於頻率
軸經此證為最優(不再是假設)。** 起始月份掃描:秋季(9-12 月)這段史測起來優於春季
(9 月 99.6% vs 3 月 86.4%),但**樣本僅 ~10 refit 點、單一史 → 過擬合風險高,列為
弱訊號不據以改月**;現行 7 月(95.3%)穩健可接受。**方法極限**:參數層 meta(24-config
網格,同 M01 限)、連續 NAV 切片近似、小樣本——方向性結論(年更 > 月更/永不)由雙
KPI 單調一致支撐,「最佳月份」不支撐。結果 → `ledger/m02_refit_frequency.parquet`。
