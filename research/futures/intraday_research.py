...
OUT_DIR = paths.OUT_STRAT_LAB / "futures_tx_intraday"
...
def _write_doc(summary: pl.DataFrame, cutoff: str, elapsed: float) -> None:
    ...
    lines += [
        ...
        "## Artifacts",
        "",
        f"- `{paths.OUT_STRAT_LAB}/futures_tx_intraday/intraday_strategy_summary.csv`",
        f"- `{paths.OUT_STRAT_LAB}/futures_tx_intraday/top_daily.csv`",
        f"- `{paths.OUT_STRAT_LAB}/futures_tx_intraday/top_trades.csv`",
    ]
    DOC_PATH.write_text("\n".join(lines) + "\n", encoding="utf-8")
