...
訊號引擎全凍結,槓桿住 NAV overlay 層(research/trading/margin_sim,制度常數
第一手查證)。選擇紀律:train 選 max P5(KPI v3),OOS 只驗不選;收編標準
= 兩折 OOS P5 均 ≥ 未槓桿 baseline。

Run: uv run --project research python -m research.evergreen.ev54_margin
依賴: tri dashboard 快取(先跑過 research.tri.pnl_dashboard)+ cache 最新
"""
