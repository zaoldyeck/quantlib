...
Run: uv run --project research python -m research.evergreen.ev28_build_pilot
"""
...
def main() -> None:
    md = open("research/evergreen/PROMPT_ev28_labeling.md").read()
    seg = md.split("## 標記提示詞", 1)[1]
    body = seg.split("\n---\n", 1)[0].split("\n\n", 1)[1].strip()
    phil = open("research/evergreen/data/ev27_phil_inline.txt").read()
    assert "{哲學全文}" in body
    ...
    open("research/evergreen/workflows/ev28_pilot.js", "w").write(script)
