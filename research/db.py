...
archives under `data/` via `research/crawl/rebuild.py`. PostgreSQL was retired
...
def connect(cache_path: str = CACHE_DB, read_only: bool = True,
    ...
    archives via `uv run --project research python -m research.crawl.rebuild --all`.
    ...
    if not os.path.exists(cache_path):
        raise FileNotFoundError(
            ...
            "請從 raw 重建:`uv run --project research python -m research.crawl.rebuild --all`"
            "(季報鏈另跑 research.crawl.rebuild_financials)。")
