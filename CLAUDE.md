...
**教訓（2026-07-16 池品質對決時發現）**：舊文曾宣稱「cache 只有 TWSE」已作廢——
...
- **sources/**:每源一個 adapter——`fetch_day(market, day)`(日頻)或
  ...
  月頻(operating_revenue/ex_right/capital_reduction/treasury)、季報(bs/is/cf)、期貨/tdcc。
- **archive.py**:`save_raw`/`save_sentinel`(tmp→os.replace),原始檔封存鐵律執行點。
- **sink.py**:單一 DuckDB upsert sink(刪該日 + 插入,idempotent)。
...
一直被塞進版控(2026-07-22 事故)。

**常用位置**:`paths.CACHE_DB`(= `var/cache/cache.duckdb`)、`paths.OUT`、
`paths.OUT_STRAT_LAB`、`paths.REPORTS`、`paths.STATE`、`paths.RAW_INTRADAY`、
...
- **月頻**:operating_revenue(含 report_date,FC8 產業別 PIT 錨)、ex_right_dividend
  (含 FC1 參考價欄 closing_price_before / reference_price)、capital_reduction、treasury_stock_buyback。
- **季報 raw base**:is_progressive_raw、bs_concise_raw、cf_progressive_raw(長表逐科目)。
...
### Prerequisites
- **uv**(Python 套件/環境管理)。**無 Java/SBT/PostgreSQL 依賴**(Scala 已封存 `legacy/scala/`、PG 已 drop)。
