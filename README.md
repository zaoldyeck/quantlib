...
- **爬取**：`src/quantlib/crawl/` 15+ 源(價量/財報/籌碼/期貨)，原始檔先封存 `data/` 才 parse。
...
| **Python 研究目錄結構** | [`src/quantlib/README.md`](src/quantlib/README.md) |
...
```
.
├── src/quantlib/                      Python 全棧（uv 專案,pyproject.toml 在此）
...
```bash
uv sync --project .          # 裝依賴（含 fubon_neo / shioaji,lock 在 uv.lock）
```
...
uv run --project . python -m quantlib.crawl.rebuild_financials   # 財報鏈 + raw_quarterly
```

### 1b. 日內 1 分 K 回補（每天跑一次）

```bash
uv run --project . python -m quantlib.intraday.pull_kbars
uv run --project . python -m quantlib.intraday.pull_kbars --status   # 看進度,不連線、不吃額度
```
...
```bash
uv run --project . python -m pytest -q          # 全套件（251 tests）
```
...
財務品質因子（F-Score / cfo_ni）由 `var/cache/raw_quarterly.parquet` 提供,`db.connect()`
...
- **讀取一律 `quantlib.db.connect()`** — cache-only,查詢自帶 `WHERE market=...`。
