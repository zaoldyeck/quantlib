export const meta = {
  ...
  whenToUse: '全 Python 化:port Scala reader → research/crawl/sources/',
...
重寫成 Python,放進現有框架 research/crawl/sources/。
...
5. **既有 port 樣板**:research/crawl/sources/daily_quote.py(fetch → archive → parse
   → Sink 的標準形態)、research/crawl/sink.py、research/crawl/http.py、
   research/crawl/parse.py、**research/crawl/archive.py**(原始檔封存鐵律)。

## 產出(寫進 repo,不要只描述)
1. **research/crawl/sources/${u.source}.py**:
...
2. **research/crawl/tests/test_${u.source}_parity.py**:
