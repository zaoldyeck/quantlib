...
### 單元測試
```bash
uv run --project . pytest src/quantlib/tests/ -v
```

### 資料 audit（CLI 個別執行）
```bash
uv run --project . python src/quantlib/audits/01_find_spikes.py --min-gain 0.80 --window 60
uv run --project . python src/quantlib/audits/02_anomaly_scan.py --min-stocks 20
# 03/04/05 同樣模式
```

### 主動 ETF 績效分析
```bash
uv run --project . python src/quantlib/analyses/active_etf_metrics.py
```
