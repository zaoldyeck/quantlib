...
錯誤才停;額度於**開盤日** 08:00 重置(官方明文,**非自然日**——週末不重置),
launchd 每日 08:30 自動續跑(非交易日跑到的是前一交易日的餘額)。

**兩道官方限制,綁住的是不同東西(2026-07-21 實測 + 官方文件)**
  流量:**依近 30 日成交金額分級**(2026-07-26 查證官方文件 + 實測 usage()):
        成交 0 元 → **500 MB**(= 524,288,000 bytes,**目前本帳戶所在級距**);
        1 元~1 億 → 2 GB;>1 億 → 10 GB。**這才是總工期的瓶頸**:每格約 321 KB,
        全市場 2,395 檔 × 77 個月 ≈ 184,415 格 → 500MB 級約 **4 個月**、2GB 級約 1 個月。
        ⚠ **只要開始實際交易(成交金額 ≥1 元)額度即升 4 倍**——目前每檔買 1 股的
        測試模式讓帳戶停在最低級距。
  頻率:行情類 **10 秒 50 次**(官方;盤中 kbars ≤270 次、ticks ≤10 次)——綁的是
        「每天那一輪跑多久」。序列實測 1.76 格/秒
...
用法:
  uv run --project . python -m quantlib.intraday.pull_kbars             # 續傳回補
  uv run --project . python -m quantlib.intraday.pull_kbars --status    # 進度(不連線)
  uv run --project . python -m quantlib.intraday.pull_kbars --gaps      # 缺哪幾天(不連線)
