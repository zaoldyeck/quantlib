...
#: **已知但刻意不特殊處理的例外**:2008-10-13 ~ 2008-10-24 台股**跌幅減半為 3.5%**
#: (金融海嘯期間的臨時措施;由 EV58 的 E1 年代語境卡回報,已自資料驗證——該窗內
#: 當日最低報酬恰為 −0.0350 且零檔跌逾 3.6%,窗前窗後皆為 −0.07 且數百檔觸及)。
#: 為什麼不改判準:實測該窗 7,068 筆觀測中,「跌幅落在 −7% ~ −3.5% 之間」的只有
#: **1 筆**——用較嚴的界限只多抓一筆,而多一條時窗特例就多一處日後會漂移的地方。
#: 判準保持單一,事實記在這裡。
HALVED_DOWN_LIMIT = (Date(2008, 10, 13), Date(2008, 10, 24), 0.035)
