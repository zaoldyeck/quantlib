...
def _build_settlement(con, plan, holdings: dict[str, float], cash: float):
    ...
    try:
        ...
        from quantlib.trading.cost_basis import BASIS_LABEL, cost_of
        from quantlib.trading.live.money import build_settlement
...
def main() -> None:
    ...
    if not args.no_refresh:
        from quantlib.crawl.update import ensure_fresh
        ensure_fresh()
    ...
    try:
        from quantlib.trading.live import safety_net
        if args.positions is not None:
    ...
    try:
        from quantlib.apex import refit
