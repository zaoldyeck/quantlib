...
from quantlib.trading.order_planner import (
    build_order_plan,
...
from quantlib.trading.portfolio import apply_trade_fills, fills_from_order_plan, inventory_mismatches
from quantlib.trading.strategy_registry import StrategyRegistration, StrategyStage
...
def test_build_order_plan_uses_managed_positions_and_sells_first(tmp_path: Path) -> None:
    cache = tmp_path / paths.CACHE_DB.name
    make_cache(cache)
    targets = tmp_path / "targets.csv"
    targets.write_text(
        "date,company_code,target_weight\n"
