...
兩支 CLI:**買入** `quantlib.trading.execution.buy`、**賣出** `quantlib.trading.execution.sell`。
...
```bash
# 離線自測(不需憑證,驗證邏輯)
uv run --project . python -m quantlib.trading.execution.buy --selftest

# dry-run(真實行情、模擬成交、不送單;預設模式)
uv run --project . python -m quantlib.trading.execution.buy --code 2408 --qty 1
...
uv run --project . python -m quantlib.trading.execution.place_limit \
...
uv run --project . python -m quantlib.trading.execution.archive_candles [--codes 6488,3026]

# 列出/撤銷帳上未完成委託
uv run --project . python -m quantlib.trading.execution.cancel_all [--code X] [--live]

# 每日 loop 一體派工(產完 plan 自動派執行器;賣腿依出場理由分流 urgency)
uv run --project . python -m quantlib.serenity.daily run --execute        # dry-run
uv run --project . python -m quantlib.serenity.daily run --execute-live  # 真實(三閘仍在)
```
...
測試:`uv run --project . python -m pytest src/quantlib/tests/test_execution_resilience.py`
...
`git show 9d8b64c:src/quantlib/trading/legacy/smart_execution_v0.py`;
