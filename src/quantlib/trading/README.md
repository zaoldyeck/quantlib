...
(`src/quantlib/serenity/daily.py`).

**盤中執行(2026-07-09 起)**:實際下單由 `src/quantlib/trading/execution/`
...
```bash
uv run --project . python -m quantlib.trading.auto_trader status
...
uv run --project . python -m quantlib.trading.auto_trader capital-check
uv run --project . python -m quantlib.trading.auto_trader plan
...
uv run --project . python -m quantlib.trading.auto_trader run-daily
```
