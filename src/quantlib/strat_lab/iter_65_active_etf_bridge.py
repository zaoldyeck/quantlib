"""iter_65 - production core plus active-ETF-beater bridge.

Iter63 is production-grade but wins only 13/17 active ETF same-window tests.
Iter64 wins 17/17 but is not full-cycle production after cumulative DSR. This
pass tests a narrow bridge: keep Iter63 as the default core and temporarily
promote into the Iter64 high-firepower sleeve only when technology leadership
and trailing sleeve evidence agree.
"""
...
from iter_64_active_etf_beater_confirm import (  # noqa: E402
    ACTIVE_ETFS,
    CUMULATIVE_TRIALS as ITER64_CUMULATIVE_TRIALS,
    compare_active_etfs,
    load_active_etfs,
    strict_dsr,
    window_metrics,
...
MIN_HOLDS = [10, 20, 40]
CONFIRMS = [1, 2]
FOCUSED_TRIALS = len(ATTACKS) * len(GATES) * len(SCHEDULES) * len(LOOKBACKS) * len(MARGINS) * len(MIN_HOLDS) * len(CONFIRMS)
CUMULATIVE_TRIALS = ITER64_CUMULATIVE_TRIALS + FOCUSED_TRIALS
...
def load_base(attack: Sleeve) -> pl.DataFrame:
...
def simulate(base_raw: pl.DataFrame, spec: Spec) -> pl.DataFrame:
    ...
    core_ret = base["ret_core"].to_numpy().astype(float)
    attack_ret = base["ret_attack"].to_numpy().astype(float)
    ...
    for i in range(len(dates)):
        ...
        r = attack_ret[i] if state == "attack" else core_ret[i]
        if did_switch:
                    ...
                    for margin in MARGINS:
                        for hold in MIN_HOLDS:
                            for confirm in CONFIRMS:
                                name = (
                                    f"iter65_core63_{attack.key}_{gate}_{schedule}"
                                    f"_lb{lookback}_m{int(margin * 100)}_hold{hold}_confirm{confirm}"
                                )
                                specs.append(Spec(name, attack, gate, schedule, lookback, margin, hold, confirm))
    return specs
...
def main() -> None:
    specs = build_specs()
    print(
        f"[iter65] specs={len(specs)} active_etfs={len(ACTIVE_ETFS)} "
        f"focused_trials={FOCUSED_TRIALS} cumulative_trials={CUMULATIVE_TRIALS}",
        flush=True,
    )
    bases = {attack.key: load_base(attack) for attack in ATTACKS}
    ...
    for i, spec in enumerate(specs, 1):
        daily = simulate(bases[spec.attack.key], spec)
        ...
        row = {
            ...
            "max_active": max(CORE.max_active, spec.attack.max_active),
            **stats,
            **window_metrics(daily, 365),
            **active_summary,
        }
        row["screen_rank"] = (
            row["active_etf_wins"] * 100.0
            + row["active_etf_min_gap"] * 10.0
            + row["oos_sortino"]
            - max(-row["oos_mdd"] - 0.35, 0.0)
        )
        rows.append(row)
        compare_rows.extend(active_rows)
        if i % 40 == 0:
            print(
                f"[iter65 screen] {i:03d}/{len(specs)} wins={row['active_etf_wins']:.0f}/{row['active_etf_count']:.0f} "
                f"OOS={row['oos_cagr']:+.2%} Sortino={row['oos_sortino']:.3f} "
                f"1Y={row['recent_1y_cagr']:+.2%} attack={attack_pct:.1%}",
                flush=True,
            )

    screen = pl.DataFrame(rows).sort(
        ["active_etf_wins", "active_etf_min_gap", "oos_sortino", "oos_mdd"],
        descending=[True, True, True, True],
    )
    selected_names = set()
    for selected_frame in [
        screen.sort(["active_etf_wins", "active_etf_min_gap", "oos_sortino"], descending=[True, True, True]).head(VALIDATE_TOP_N),
        screen.sort(["active_etf_wins", "oos_sortino", "oos_cagr"], descending=[True, True, True]).head(VALIDATE_TOP_N),
        screen.sort(["active_etf_wins", "oos_sharpe", "oos_cagr"], descending=[True, True, True]).head(VALIDATE_TOP_N),
        screen.sort(["active_etf_wins", "recent_1y_cagr", "oos_sortino"], descending=[True, True, True]).head(VALIDATE_TOP_N),
    ]:
        selected_names.update(selected_frame["name"].to_list())
    selected = screen.filter(pl.col("name").is_in(selected_names))
    print(f"[iter65] full-validation candidates={selected.height}", flush=True)
    final_rows = []
    ...
    summary = pl.DataFrame(final_rows).sort(
        ["strict_promotable", "focused_promotable", "beats_all_active_etfs", "active_etf_wins", "oos_sortino"],
    ...
    print(f"[iter65] wrote {screen_path}", flush=True)
    print(f"[iter65] wrote {summary_path}", flush=True)
    print(f"[iter65] wrote {compare_path}", flush=True)
