...
Run:
    uv run --project . python src/quantlib/strat_lab/validate_hybrid.py [tag]
...
def deflated_sharpe(sr_annual: float, n_trials: int, daily_rets: np.ndarray) -> float:
    ...
    **正解**:`src/quantlib/apex/validate.py::deflated_sharpe(nav, n_trials, sr_var_across_trials)`
...
def pbo_cscv(folds: list[dict], n_trials: int = N_PBO_SPLITS) -> float:
    ...
    **正解**:`src/quantlib/apex/validate.py::pbo_cscv(returns: T×K, s=16)`——需把同一 campaign
