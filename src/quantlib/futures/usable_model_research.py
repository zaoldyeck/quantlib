...
OUT_DIR = paths.OUT_STRAT_LAB / "futures_tx_usable_model"
...
def _write_doc(summary: pl.DataFrame, cutoff: str, elapsed: float, pbo: float) -> None:
    ...
    lines += [
        ...
        "## Artifacts",
        "",
        f"- `{paths.OUT_STRAT_LAB}/futures_tx_usable_model/usable_model_summary.csv`",
        f"- `{paths.OUT_STRAT_LAB}/futures_tx_usable_model/top_daily.csv`",
        f"- `{paths.OUT_STRAT_LAB}/futures_tx_usable_model/top_trades.csv`",
    ]
    DOC_PATH.write_text("\n".join(lines) + "\n", encoding="utf-8")
