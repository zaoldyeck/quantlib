...
安聯台灣科技基金(見 `src/quantlib/tri/allianz_fund.py`)。

**誠實基準(2026-07-20 重構)**:三策略線一律呼叫各自的官方引擎(Evergreen=
`quantlib.evergreen.engine`、S=`chart_s_vs_benchmarks.run_s`、Serenity=`serenity/engine.py`),
...
上圖 NAV(log)/下圖 DD、KPI 卡、逐年表。由 `quantlib.tri.daily` 鏈尾自動重生
(--no-dashboard 跳過)。

Run: uv run --project . python -m quantlib.tri.pnl_dashboard
...
def _cache_latest() -> date:
    ...
    from quantlib.apex import data as apex_data
...
def serenity_nav_trades(end: date) -> tuple[pd.Series, pd.DataFrame]:
    ...
    def _compute():
        subprocess.run([sys.executable, "-m", "quantlib.serenity.wf.build_registry"],
                       cwd=REPO_ROOT, check=True, capture_output=True, text=True)
        cmd = [sys.executable, str(REPO_ROOT / "src" / "quantlib" / "serenity" / "engine.py"),
               "--start", START.isoformat(), "--end", end.isoformat(),
               "--registry", str(REPO_ROOT / "src/quantlib/serenity/wf/registry_wf.csv"),
...
def evergreen_nav_trades(end: date) -> tuple[pd.Series, pd.DataFrame]:
    ...
    **零重寫**:池籍/計分/simulate 一律呼叫 quantlib.evergreen.engine(唯一真源;
    ...
    from quantlib.apex import data as apex_data
    from quantlib.evergreen.engine import walkforward_cached
...
def s_nav_trades(end: date) -> tuple[pd.Series, pd.DataFrame]:
    ...
    def _compute():
        from quantlib.apex import data as apex_data
        from quantlib.apex.strategy_s import prep, run_s_full  # 官方 S 引擎唯一真源
        con = apex_data.connect()
        try:
...
def build_html(navs: dict[str, pd.Series], trades: dict, data_date: date) -> str:
    ...
    note = (f"資料日 <b>{data_date}</b> · 生成 {date.today()} · 三策略均自 <b>{START}</b>"
            f"(起始標記池)起 · 基準含息調整 · 由 <code>quantlib.tri.daily</code> 自動更新"
...
def main() -> None:
    from quantlib.tri.allianz_fund import load_nav as allianz_nav
