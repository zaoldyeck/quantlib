...
規格來源:S = src/quantlib/apex/STRATEGY.md(逐條移植;per-position 狀態存
var/state/positions/,cost=收養日收盤);Evergreen v3.3 = LEDGER EV30-33
(參數外部化:`src/quantlib/evergreen/data/live_config.json`,EV43 年度 refit 產物);
Serenity = live ledger 錨 × `src/quantlib/serenity/exit_rules.py`(與執行系統
同一份六道門規則源)。
"""
...
from quantlib.trading.cost_basis import cost_of, levels_line
...
def s_advisor(con, holdings: dict[str, float], today: Date,
    ...
    from quantlib.apex.assemble import apply_avail_override, build_features
    ...
    from quantlib.trading.exit_replay import load_paths, replay, s_rule
    ...
    for code in sorted(holdings):
        ...
        if code not in anchors:
            adv.sells.append((code, "非本策略標的(S 有紀錄以來從未選中它——它不是"
                                    "S 買的,也不會是;卡在哪一關可跑 "
                                    "`python -m quantlib.tri.pool_trace " + code + "`)"))
            continue
...
EG_LIVE_CFG = "src/quantlib/evergreen/data/live_config.json"
...
def evergreen_advisor(con, holdings: dict[str, float], today: Date,
    ...
    reg = pl.read_parquet("src/quantlib/evergreen/data/registry_v3.parquet")
    ...
    from quantlib.evergreen.engine import feat_cols, score_expr
    ...
    from quantlib.trading.exit_replay import evergreen_rule, load_paths, replay
...
def serenity_advisor(con, holdings: dict[str, float], today: Date,
    ...
    from quantlib.serenity.daily import market_data, trading_days_between
    from quantlib.serenity.exit_rules import (ABS_STOP, TAKE_PROFIT, TIME_DAYS,
                                              TRAIL, evaluate_exit)
    ...
    from quantlib.trading.exit_replay import load_paths, replay, serenity_rule
