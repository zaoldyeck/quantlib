...
|---|---|---|
| 指令 | `quantlib.tri.daily` | `quantlib.serenity.daily run` |
...
**出場一律逐日重放,不是今日快照**(`src/quantlib/trading/exit_replay.py`):規則在你沒跑
...
```bash
uv run --project . python -m quantlib.tri.daily --positions "2330:1000,2317:2000" --cash 500000
# 或 csv(欄位 code,shares):
uv run --project . python -m quantlib.tri.daily --positions-file my_positions.csv --cash 500000
```
...
| Serenity brief | `src/quantlib/serenity/out/briefs/YYYY-MM-DD.md` |
| 出場狀態(首見日/峰值,自動維護) | `var/state/positions/*.json` |
| Evergreen 標記庫 | `src/quantlib/evergreen/data/registry_v3.parquet` |
| PnL 儀表板(七線,含安聯台灣科技基金) | `src/quantlib/tri/pnl_dashboard.py` → `var/reports/tri/pnl_dashboard.html`;基金淨值來源見 `src/quantlib/tri/allianz_fund.py` docstring |
| 策略規格書 | S: `src/quantlib/apex/STRATEGY.md`;Serenity: skill `serenity-trading-system`;Evergreen: `src/quantlib/evergreen/LEDGER.md`(EV30-33 定版) |
