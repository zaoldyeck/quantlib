...
用法:
  uv run --project . python -m quantlib.tri.daily                 # 富邦 API(庫存+現金)
  uv run --project . python -m quantlib.tri.daily --positions "2330:1000" --cash 500000
...
from quantlib.tri.advisors import (Advice, evergreen_advisor, s_advisor,
...
def ensure_fresh_cache(no_refresh: bool) -> None:
    ...
    「齊備」的定義見 `src/quantlib/data_calendar.py` 與 `docs/data_ops/twse_publish_times.md`:
    **D 的資料自 D+1 00:30 起才算齊備**(融資融券官方只保證次一營業日開市前公告、借券
    ...
    with open(log_path, "w", encoding="utf-8") as lf:
        ...
        # quantlib.crawl.update 一步到位取代舊「Main update(Scala→PG)+ cache_tables(PG→cache)」。
        rc = subprocess.run(["uv", "run", "--project", ".", "python", "-m",
                             "quantlib.crawl.update", "--upto", str(want)],
...
def ensure_serenity_current(no_curation: bool) -> list[str]:
    ...
    cur_dir = REPO_ROOT / "src" / "quantlib" / "serenity" / "curation"
    ...
    if not brief_p.exists():
        ...
        r = subprocess.run(
            ["uv", "run", "--project", ".", "python", "-m",
             "quantlib.serenity.daily", "run", "--skip-refresh"],
...
def main() -> None:
    ...
    from quantlib.tri.report import action_block, stock_appendix, stock_card
    ...
    if not args.no_dashboard:
        ...
        rd = _sp.run([sys.executable, "-m", "quantlib.tri.pnl_dashboard"],
