...
| 路徑 | 內容 |
|---|---|
| `daily.py` | **每日營運入口**:`uv run --project . python -m quantlib.serenity.daily run`(資料刷新 → 引擎 → live book → 訂單計畫 → 日報);`override --force-exit <code> --reason ...` = 人工論點停損 |
...
- 營收首見日:`src/quantlib/records/revenue_first_seen.parquet`
