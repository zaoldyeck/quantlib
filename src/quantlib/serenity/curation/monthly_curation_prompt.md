...
# 觸發:`quantlib.tri.daily` 於每月第一次執行時 headless 自動觸發(吃訂閱額度)。
...
1. Read 檢核標準與現役冊:
   ...
   - `/Users/zaoldyeck/Documents/scala/quantlib/src/quantlib/serenity/registry/thesis_registry_2025.csv`
   - `/Users/zaoldyeck/Documents/scala/quantlib/src/quantlib/serenity/registry/member_roles.csv`
2. 機械種子(與回測同一腳本、同 PIT 語義):在 repo root 執行
   `uv run --project . python -m quantlib.serenity.backfill.seed_signals --months <M>`
   → Read `src/quantlib/serenity/backfill/seeds/<M>.json`(動能/營收加速聚類)。
...
8. 若有改冊:git add src/quantlib/serenity/registry/ && git commit(訊息含主題與證據
