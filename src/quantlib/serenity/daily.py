...
from quantlib.serenity.exit_rules import (  # noqa: E402
...
def cmd_run(args: argparse.Namespace) -> None:
    ...
    if not args.skip_refresh:
        ...
        # quantlib.crawl.update 一步取代舊「Main update(Scala→PG)+ cache_tables(PG→cache)」。
        r = sh(["uv", "run", "--project", ".", "python", "-m",
                "quantlib.crawl.update"], timeout=3000)
        if r.returncode != 0:
    ...
    r = sh(
        [
            "uv", "run", "--project", ".", "python", str(ENGINE),
    ...
    from quantlib.trading.exit_replay import load_paths as _load_paths
    from quantlib.trading.exit_replay import replay as _replay
    from quantlib.trading.exit_replay import serenity_rule as _serenity_rule
    ...
    r = sh(
        ["uv", "run", "--project", ".", "python", "-m", "quantlib.trading.auto_trader", "plan", "--out", str(plan_out)],
        timeout=600,
    )
    brief.append("")
    if r.returncode != 0:
        ...
        brief.append(
            f"- 送單指令(人工步驟):`uv run --project . python -m quantlib.trading.auto_trader submit-plan {plan_out}`"
        )
        if getattr(args, "execute", False) or getattr(args, "execute_live", False):
            ...
            for o in orders:
                ...
                _spawn(base + ["quantlib.trading.execution.sell", "--code", code, "--qty", str(int(o["quantity"])),
