...
來源:3d5413eb-b7db-45c8-bf62-efdef11c1375.jsonl @ 2026-07-10T01:11:42.877Z(工具 Bash)
...
inst = (inst.sort([C, "date"])
        ...
        .filter(pl.col("date") >= Date(2022, 7, 1)))

def run(flag, name, guard_thr=None):
    ...
    oo = v[-1]/v[0]-1
    ledger.log_trial(family="evergreen", name=f"ev9b_{name}", hypothesis="DD鄰域/法人",
...
for thr, nm in [(None, "v3基準"), (0.10, "DD10"), (0.15, "DD15"), (0.20, "DD20")]:
    rows.append(run(flag_pool, nm, thr))
flag_inst = pl.concat([flag_pool, inst]).unique(subset=["date", C])
rows.append(run(flag_inst, "法人淨賣出場"))
