...
**campaign 依 A1-R 收斂宣告完成。** 詳見 `src/quantlib/apex/REPORT.md`。
...
**公告日資料現況(記錄)**:`src/quantlib/records/revenue_first_seen.parquet` 已上線
...
**V01 結果(2026-07-21,7 因子)**:
- **晉級:`cfo_yield`(CFO_ttm/市值,t' 6.8/4.7、spread +10.5/8.9%、mono 0.84)——
  現金流品質第三次確認(cfo_ni/cfo_ta 同族,估值版)**。
- tilt(IC 高、top decile 反弱):ep(t' 7.5 但 spread **−3.9~−5.8%**)、ev_ebit_inv
  (同型)——REPORT §5 預言確認,集中組合用不上。bp 邊緣;dcf_proxy IC 型
  (t' 6.7、mono 0.3-0.4,g1 假設污染 tail);peg_inv 死(spread −12%)。
- **fiveline_z_neg(五線譜)壯烈反向:spread −24.8%、mono −1.00 完美反指標**——
  「低於趨勢線買」在台股個股層買到的是下跌趨勢股;反向用=動能族已有。
  使用者點名的五線譜正式證偽(個股截面)。

**V03 結果(2026-07-21):cfo_yield 嫁接慘敗**——S+cfo_yield^0.5 56.9/P5 24.9、
閘替換 50.6/17.7 vs 基準 120.9/67.3(重現 ✓)。估值軸把「貴的動能股」降權=
直接砍 alpha;條件化教訓五度重現。V-LINE 收官:估值模型全譜對 S 無增量。

**T01 結果(2026-07-21,9 因子):零晉級**。rsi14/stoch_k14/boll_pctb/mfi14/
obv_slope20/vwap20_dev/avwap_dev 全部 tail 家族(spread +10~28%、mono ~1.00、
t' <2.3)——右尾力與 52wH/close_pos/updays 同族且不更強,無新資訊;
boll_bw_neg(squeeze)反向 tail(−14.5%、mono −0.93)= lowvol 毒近親;
macd_hist = 短窗動能換皮(h5 一瞬即逝)。預測全命中:oscillator 在動能市
「超買」為右尾特徵(反教科書)。傳統技術指標線收官,無嫁接候選。
...
**T02/V02 結果(2026-07-21):容器全滅**。海龜 55/20 全期 3.7%/P5 −0.07/MDD
−58.6%;20/10 **−4.5%/MDD −88.1%**(漲跌停市場突破 whipsaw + 2N 停損連環觸發,
比 N01 月頻動能更慘——突破容器 < 排名容器再確認);**神奇公式 N=20 全期 −2.4%/
MDD −63.5%**——非 bug,與 V01 內部一致:ep top decile 為負(台股景氣循環股
「低 PE=獲利頂點」陷阱),重倉高 EY=買在循環頂。舊 Scala mf_piot 較好看的差異
= Piotroski 閘擋掉陷阱股 + TWSE-only 月頻(不同宇宙),pure Greenblatt 在台股
雙市場低門檻池死刑。亞當理論:N01(P5 12.8)+ 本批 donchian 變體雙證,純價格
慣性容器天花板遠低於 S。

# Q/V/T-LINE 終掃收斂宣告(2026-07-21)

**三線 6 批 + 2 嫁接批,~50 變體,新增 33 因子 IC 廣篩:0 個過線。**
- IC 層真發現 4 個(gm_vol8/ni_vol8/cfo_ta/cfo_yield——「財務穩健」的可交易形態
  =穩定性+現金流,非水準分數),但嫁接 S 全敗 0/7(條件化教訓五度重現:S 的
  揭露事件 cohort 內,防禦/估值慢變數=稀釋進攻 alpha)。
- 容器層全滅(海龜/神奇公式/純財務三書);純財務書天花板 9-17%(方向 2/3 科學
  答案:能賺、遠遜含事件時機的 S;穩定書 = Sharpe 1.01 防禦之王)。
- 使用者點名項全數處置:F/Z/O/杜邦 ✓(Q01)、Beneish 不可算聲明 ✓、SMC/TPO/
  VWAP/形態 ✓(F03 舊+T01;TPO 無日內資料聲明)、DCF/PEG/五線譜 ✓(V01;
  五線譜 mono −1.00 完美反指標)、海龜/亞當/神奇公式 ✓(T02)、由大到小+網格 ✓
  (0 過線 → 依預註冊無網格對象)、純財務成長/品質 ✓(Q02)。
**疊加先前 8 線攻防(F/G/L/N/X/R/M),S 王座 11 線全勝。收斂宣告:資訊集內
(日 OHLCV + 月營收 + 季報)的因子與結構空間已系統性掃空,「S 已優化到極限」
於本輪全譜系擴充下成立。** 更強的剩餘可能性全在資訊集外:真實揭露時點資料
(revenue_first_seen 累積中,F04 死因 (a) 的解方)、日內資料(使用者閘)、
成本結構改變(G05 ML 臂復活條件)。

## X02 — S × 穩定書組合前緣(2026-07-21;使用者:「不優化 S,組合/從頭研發超越?」)

「從頭研發」= F-LINE 制度性已答(自由重研發收斂回 S;M01:每年從頭選、年年選回
S 結構;年度 refit 12/1 已自動化)。「組合」的新資產缺口:Q02 穩定書為 X01 後
誕生的全自研成分(滿足 X01 裁示),量測完整前緣(2015-07→2026-07,constant-mix
日再平衡 + 波動平價變體)。

**X02 結果:組合零價值,全線劣化**。日報酬相關 0.44(高於 S×Iter95 的 0.38——
穩定書仍是 long-only 台股,市場 beta 共擔);前緣:S 純 79.7/P5 45.4/MDD −32.6/
Martin 9.3 → 50/50 48.5/30.2/−28.6/8.3 → 穩定純 20.2/8.9/−27.7/3.1;波動平價
36.6/22.6 更糟。**每讓 10% 權重給穩定書:CAGR −6pp、MDD 僅 +0.8pp、Martin 持平
微降——連風險調整改善都沒有**(對照 X01 的 Iter95 臂至少 Martin +2.1/MDD +9pp,
使用者仍判不採;本臂更無理由)。組合數學定論:要在報酬面超越 S,第二臂需
「報酬相當 + 低相關」——11 線搜索證明資訊集內不存在此臂。組合能給的最多是
「更平滑」,在本資訊集內連這都給不出超越 X01 的版本。
...
## M03 — refit 時點精修(2026-07-21;使用者三問:為何非 1/1、12 月何時、現役 = Dec-2025?)

`experiments/m03_refit_timing.py`(reuse config NAV + m02 helpers)。**(A) 各年 12/1
trailing-3yr 選到的 config**:2010-2013 ax4-n5(小池)→ 2013-2018 ax4-n5-t35-**adv20**
(4 因子大池)→ 2019 過渡 → **2020-2025 全部 = ax6-n5-t35-adv5 = S(連 6 年)**。
**關鍵:Dec-2025 窗(2022-12~2025-12)選到 S、實際現役 Jul-2026 窗也選到 S = 同一支**
→ **現役 S 就等於「去年 12 月版」,參數不用動**;且早年 config 真的換過 → 再證「永不
更新會卡舊版」。**(B) 12 月日內時點**(月營收 ~10 日公布):day 1/6/11(月報前後)
68.1/68.9/68.5% ≳ day 16/21(月報後)67.2/67.3%——**月報前的早 12 月略優、峰在 6 號**,
但整個上旬差 ~1.5pp(小)。**反直覺一致性**:含最新單月營收(月報後、或 1 月 vs 12 月)
反而略差 = 最新單月營收是噪音、塞進 3 年選擇窗添亂。**定案:refit 固定 12/1**(使用者
拍板;上旬皆可、取月初乾淨錨)。機制澄清:月份效應主要來自「接住 config 轉變的時機」,
穩定期(現在 S)月份不改參數,價值在下次變天時抓得準。

## N04/N05 — 條件機率地圖 → 資金分配軸(2026-08-07 預註冊)

**緣起**:使用者要求把「對數報酬建模 + 方向×幅度編碼 + 條件機率 + 嚴格 OOS」這套
思維導入 S。做法不是猜規則再回測,而是**先建條件期望地圖、讓地圖指出該改哪裡**。
地圖 = `experiments/n04_conditional_map.py`(S 候選首次進榜 1,255 筆 × 未來 60 日
= 61,938 個持倉日觀測;狀態全部以進場時 20 日日波動 σ 標準化;IS/OOS 依進場日 75/25)。

**N04 已得(三條,全部 IS/OOS 對照)**:
1. **固定 35% trail 在機率上不一致**:進場 σ 中位 3.10%(P10 1.16%、P90 5.35%)
   → 35% 回落 = 低波股 37σ、中位股 13.9σ、高波股 8.1σ,**同一條線代表 4.5 倍不同的
   極端程度**。σ 標準化的回落分桶,OOS 期望值排序比原始 % 分桶乾淨(σ 版單調至 bk4,
   % 版在 bk3 塌陷後又跳回)。
2. **榜內名次不帶前瞻資訊**:top-5 內 rk1~rk5 的未來 10 日期望 IS 133/163/66/122/56、
   OOS **−150**/188/93/66/130,兩窗皆非單調、OOS 的第一名最差(t −3.98,n=1,078)。
   → **等權 20%×5 有理據;依分數名次加權 = 擬合雜訊。此軸關閉。**
3. **波動分層的風險調整期望單調**:E[fwd10]÷σ 由低波到高波,IS 0.696/0.356/0.438/
   0.354/0.141、OOS 0.945/0.019/0.330/0.404/0.024——**最低波動五分位在兩窗都最佳、
   最高波動五分位在兩窗都墊底**(裸期望值差異小:IS 78 vs 74、OOS 140 vs 14 bp)。

**N05 假設(本批要證的)**:既然 S 的高波動候選「賺一樣多但用掉數倍風險」,把資金
由高波動倉移向低波動倉應改善風險調整後績效。**但這與 S 的血統相衝**——REPORT §8d
明載 S 的超額來自「微型股營收火箭」的右尾,逆波動加權會系統性減碼正是那些火箭。
故本批同時掛上反方向臂,由資料裁決,不由直覺裁決。

**臂**(全部只動 `score_fn` 產出的 `weight` 欄,引擎零改動;σ_ref = 當日候選池 σ 中位,
PIT 安全):
- A0 equal   等權 20%(對照)
- W1 shrink  w = 0.20 × min(1, σ_ref/σ),clip [0.08, 0.20]——只縮高波、不加碼
- W2 parity  w = 0.20 × (σ_ref/σ),clip [0.10, 0.35]——完整逆波動
- W3 cut_hi  剔除當日 σ 最高五分位候選(池瘦身,須先扣 N03 量到的 −13~−14pp 砍池成本)
- W4 lean    w = 0.20 × (σ/σ_ref),clip [0.10, 0.35]——正波動加權(右尾假說的直球對決)

**判準**:主排序沿用 v3 KPI 精神——CAGR 與 Sharpe **同時不劣於 A0** 才算有價值;
只升 CAGR 而 Sharpe/MDD 惡化 = 換到槓桿不是 alpha,不採用。任一臂勝出須再過
IS/OOS 分窗一致性(勝出方向在兩窗同號)才進入正式驗證。

**已知限制(先寫下,免得事後解釋成發現)**:N04 的持有天數 × σ 回落交叉表有 49 格,
「IS/OOS 同號為負」的格子(k 16-20 × 回落 −1~−3σ)在 49 格的多重比較下用隨機也挑得到,
**故不列為 N05 的假設**;要動它必須先有能表達「部位年齡 × σ 回落」的引擎原語,另案。

### N05 結果(2026-08-07):資金分配軸全敗,判準未過

| 臂 | 全窗 CAGR | Sharpe | MDD | 前窗 ΔSharpe | 後窗 ΔSharpe |
|---|---|---|---|---|---|
| A0 equal 等權 20% | 84.17% | 2.109 | −39.7% | — | — |
| W1 shrink 只縮高波 | 43.67% | 2.103 | −24.7% | −0.011 | −0.010 |
| W2 parity 完整逆波 | 47.98% | 2.086 | −25.5% | +0.019 | −0.094 |
| W3 cut_hi 剔高波五分位 | 30.37% | 1.360 | −36.8% | −0.613 | −0.888 |
| W4 lean 正波動加權 | 81.99% | 1.735 | −51.5% | −0.390 | −0.336 |
| W5 parity_norm 曝險歸一逆波 | 68.28% | 2.079 | −36.3% | −0.017 | −0.050 |

**裁決:五臂全部不過判準**(CAGR 與 Sharpe 需同時不劣於 A0)。W1/W2 的 Sharpe 幾乎
不動而 CAGR 與 MDD 同比例縮小,**這是降槓桿的指紋、不是風險平價的效果**;故補跑
W5(權重在當日榜上前 5 名內正規化成合計 100%,真的把資金移轉而非抽走)——W5 三窗
Sharpe 皆略負(−0.017/−0.050/全窗 −0.030),證實**不是曝險設計的問題,是這條軸本身
沒有 alpha**。W2 唯一的正號(前窗 +0.019)在後窗翻負,未過分窗一致性。

**機制**:N04 量到的「低波候選 E÷σ 較高」是**逐倉的風險效率**,而 S 是 5 席集中的
右尾策略,組合層績效由贏家的**幅度**決定,不由單倉風險效率決定。減碼高波倉正好減碼
了製造右尾的那些倉(REPORT §8d:微型股營收火箭),CAGR 掉 16~54pp 遠大於 MDD 改善。
**再次印證 F01 的老教訓:IC/條件期望顯著 ≠ 組合可用。**

## N06 — σ 標準化 trailing stop(2026-08-07 預註冊)

**假設**:N04 量到固定 35% trail 對進場日 σ 不同的股票是 8.1σ ~ 37σ(4.5 倍差),
**機率上不一致**。若把 trail 改成「固定的 σ 倍數」,同一條線對每檔股票代表同樣的
極端程度,應優於固定百分比。k=14 是關鍵臂:σ 中位 3.10% × 14 ≈ 43%,與現行 35%
同量級,**平均鬆緊不變、只把不一致修掉**——這才是「標準化本身有沒有價值」的乾淨檢定。

**引擎原語(本批新增)**:`ExitSpec.trailing_stop_col` — 逐倉 trail 幅度,進場當下讀
一次鎖進部位;與 `trailing_stop` 二擇一;欄值缺失 = 該倉無 trailing(不以全域值頂替)。
守護 `tests/test_engine.py` 三支(含突變驗證:關掉原語後逐倉臂即紅)。

**臂**:A0 固定 35%(對照);K06/K10/K14/K20 = trail 為 6/10/14/20 個 σ,
clip [0.10, 0.90](物理與防呆界限:>90% 永不觸發、<10% 純換手,非調參)。
其餘規格不動(time_stop 30、loser_time_stop 15、5 席、max_new 2)。

**判準**:同 N05——CAGR 與 Sharpe 同時不劣於 A0,且勝出方向須過前後窗一致性。

**已知反向證據(先寫下)**:B03 曾掃過固定 trail 25/35/45/None = 28.9/31.2/26.4/18.3%
(b02f config、N=20 時代),35% 為內部最優。本批不與之矛盾——B03 掃的是**固定百分比
的水準**,本批問的是**同一水準下改用 σ 尺度是否更好**,兩者是不同的問題。

### N06 結果(2026-08-07):不過判準;但診斷欄推翻了假設的前提

| 臂 | 全窗 CAGR | Sharpe | MDD | **trail 出場占比** | 前窗 ΔCAGR | 後窗 ΔCAGR |
|---|---|---|---|---|---|---|
| A0 fixed35 | 84.17% | 2.109 | −39.70% | **0.6%** | — | — |
| K06 σ×6 | 80.71% | 2.070 | −39.11% | 5.8% | −6.30pp | −0.50pp |
| K10 σ×10 | 84.42% | 2.110 | −39.35% | 0.4% | −1.20pp | +1.71pp |
| K14 σ×14 | 84.56% | 2.113 | −39.35% | 0.1% | −0.93pp | +1.71pp |
| K20 σ×20 | 85.01% | 2.119 | −39.35% | **0.0%** | −0.04pp | +1.71pp |

**裁決:不過判準**——K10/K14/K20 全窗微正(+0.25~+0.84pp / ΔSharpe +0.001~+0.010)
但**前窗全為負**,未過分窗一致性;K06(最緊)兩窗皆負,方向明確。

**比裁決更重要的是診斷欄**:**A0 的 trail 出場只佔 0.6%(675 筆裡 4 筆)**,中位持有
16 個交易日——**S 的出場其實由 loser_time_stop 15 與 time_stop 30 主宰,trailing stop
幾乎不觸發**。K20(trail 占比 0.0% = 等同無 trailing)是最佳臂,亦即 trail 對 S 的
淨貢獻在本樣本上不可辨識。

→ **N04 的「35% = 8.1σ~37σ 機率不一致」在數學上成立、在營運上幾乎無關**:一條每 170
筆才觸發一次的閘,把它調得再一致也改變不了績效。**先量閘門的觸發率,再花力氣調它的
門檻**——這是本批最可複用的教訓。**不建議移除 trail**:ledger 已記 2025-04 關稅崩盤
最慘 8 筆全走 trail 出場,低觸發率的閘可能是尾部保險,要判它生死需崩盤條件下的專門
檢定,不是全樣本平均。

### N04【8】右尾機率:解釋了 N05 為何必敗(2026-08-07)

進場 σ 五分位 → P(持有 60 日內最大漲幅 ≥ +50%):
IS 2.1% / 4.2% / 12.8% / 16.8% / **23.3%**;OOS 11.3% / 9.7% / 21.0% / 27.4% / **33.9%**。
下檔 P(跌逾 30%):IS 0.5%→14.8%、OOS 1.6%→12.9%。**高波動候選的火箭機率是低波動的
3~11 倍,而崩跌機率只有 8~26 倍分之一的增幅**(賠率約 1.6~2.6 : 1,不對稱)。

**同一份資料、兩個量尺、相反建議**:E[fwd]÷σ 說買低波(N04【6】),P(大贏)說買高波
(本節)。**對 5 席集中的右尾策略,正確的量尺是後者**——組合績效由贏家的幅度決定,
不由單倉的風險效率決定。教科書的風險平價在這裡是錯的量尺,N05 五臂全敗即其實證。
**機率思維的價值不在套用標準統計量,在先問「這個策略賺錢的機制是什麼、該量哪個機率」。**

## N07 — 出場理由普查(2026-08-07):前兩輪調的是只決定 1.3% 出場的旋鈕

| 出場理由 | 筆數 | 筆數佔比 | 報酬貢獻佔比 | 平均報酬 | 勝率 | 中位天數 |
|---|---|---|---|---|---|---|
| **signal**(營收訊號過期 stale≥26) | 472 | **69.9%** | **+134.5%** | +11.63% | 73.7% | 16 |
| **time_loser**(輸家滿 15 日) | 193 | **28.6%** | −35.9% | −7.58% | 4.2% | 16 |
| time(滿 30 日) | 5 | 0.7% | +4.7% | +38.07% | 100% | 31 |
| trail(回落 35%) | 4 | 0.6% | −3.3% | −33.24% | 0% | 12.5 |
| delist | 1 | 0.15% | −0.06% | −2.45% | 0% | 4 |

**這推翻了 N06 當時的解讀**(「出場由 loser 15 與 time 30 主宰」):`time_stop 30` 只有
5 筆。真正的主控閥是 **`_stale_days=26`**(決定 70% 的出場、134.5% 的報酬貢獻),
而 N05/N06 兩輪動的 trail 與資金權重合計只決定 1.3% 的出場。**先普查再動手——
這條紀律的成本是 5 分鐘,不做的代價是兩輪白工。**

**報酬集中度**:最賺的 1%/5%/10%/20% 筆數佔全部正報酬的 9.2%/35.0%/54.1%/77.7%;
報酬 ≥ +50% 的只有 27 筆(4.0%)。右尾集中度極高,再證該優化的目標是 P(大贏)。

**營收新鮮度**(fresh 1~7 日):IS 平均報酬 8.1/6.3/3.0/6.2/7.8%、OOS 2.2/2.3/6.5/4.7/13.4%
——**無單調關係、兩窗排序不一致**,池門檻 fresh≤7 內部大致同質,不是可用的軸。

## N08 — 條件因子篩選(2026-08-07):在 S 池內、用火箭機率當量尺

**兩個維度都是新的**:F01/F02/F03 篩選跑在全市場、用平均 IC;本批跑在 **S 的條件池內**
(fresh≤7 + cfo 閘 + ADV≥5M,318,569 列 / 739 交易日 / 每日中位 424 檔)、主判準改成
**火箭提升 = P(60日內最大漲幅≥+50% | 高五分位) ÷ 同(低五分位)**。池內無條件火箭率 6.35%。

| 因子 | IS 火箭 | IS 崩跌 | OOS 火箭 | OOS 崩跌 | **賠率(火箭÷崩跌,取兩窗較差)** |
|---|---|---|---|---|---|
| **hvn_dist** | 2.65 | 0.99 | 2.61 | 1.12 | **2.33** ★ |
| **range_pos_60** | 2.31 | 0.81 | 2.31 | 1.06 | **2.18** ★ |
| rev_yoy | 2.57 | 1.16 | 2.02 | 1.24 | 1.63 |
| updays_20 | 2.28 | 1.32 | 2.09 | 1.36 | 1.54 |
| fvg_20 | 2.25 | 1.48 | 2.53 | 1.57 | 1.52 |
| frn_60 | 1.57 | 0.81 | 2.08 | 1.91 | 1.09 |
| sig20 | 10.32 | 9.16 | 10.37 | 10.07 | **1.03(純放大器)** |
| donchian_60 | 1.00 | **0.38** | 1.02 | **0.58** | 火箭無鑑別、**專砍崩跌** |
| rev_fresh_days | 1.45 | 1.84 | 1.58 | 1.35 | 0.79 |
| dy | 0.26 | 0.29 | 0.27 | 0.18 | 反向 |
| lowvol_60 | 0.09 | 0.10 | 0.09 | 0.06 | 反向(sig20 鏡像) |

**三條結論**:
1. **sig20 是對稱放大器,不是 alpha**:火箭 10.3× / 崩跌 9.2×,絕對增幅 +10.2pp
   對 +10.1pp。→ 把波動放進計分或權重都只是加槓桿(N05 W4 的 CAGR 幾乎不變而
   Sharpe 大跌 −0.375,正是這個指紋)。**此軸正式關閉。**
   ⚠ 同時修正 N04【8】的過度解讀:那裡量的是**已被六因子篩過的持倉**,在那個子集裡
   波動才帶來不對稱賠率(絕對增幅火箭 +21.2pp vs 崩跌 +14.3pp);全池上是對稱的。
   **把「純放大器」變成「有賠率的放大器」的是那六個因子本身。**
2. **hvn_dist 與 range_pos_60 是真候選**:火箭 ↑2.3~2.7 倍而崩跌幾乎不動(0.81~1.12),
   兩窗高度一致。兩者都**不在** S 現役六因子內;F01 當年用平均 IC 判準時,hvn 版因
   Sharpe 低而落選、range_pos_60 未晉級——**換一把尺就浮出來,正是「沒在正確條件下、
   用正確量尺篩過的因子等於沒篩」的實證**。
3. **donchian_60 是防禦因子不是攻擊因子**:火箭提升 1.00/1.02(零鑑別力)但崩跌提升
   0.38/0.58(強力避開崩跌)。攻擊項與防禦項該分開評,用同一把尺會兩邊都埋沒。

## N09 — 三大主控閥掃描(2026-08-07):三軸全部已在最適點,零提升

由 N07 普查定位:先前兩輪(N05 資金分配、N06 σ trail)動的旋鈕合計只決定 1.3% 的出場。
本批掃真正在管事的三個:stale_days(70% 出場)、fresh_days(池入口)、loser_time_stop(29%)。
各軸單獨掃(不做全網格,避免多重比較把雜訊掃成發現)。

**S 軸 stale_days**(現行 26):**尖銳的內部最佳,兩窗一致**。
全窗 ΔCAGR:18 → −33.28pp、22 → −6.16pp、**26 = 0**、30 → −24.74pp、34/40 → −48.19pp;
後窗 −37.25 / −10.50 / **0** / −10.90 / −17.22pp。signal 出場占比隨門檻由 99% 掉到 0%
(門檻放寬 → 訊號死亡永不觸發 → 全交給時間止損)。**26 確認為最適,非慣例。**

**F 軸 fresh_days**(現行 7):**平坦峰,未過一致性**。
前窗 ΔCAGR 3/5/7/9/12 = −15.51 / −1.57 / **0** / −2.74 / −2.28pp;
後窗 = −22.44 / −4.10 / **0** / **+1.05** / **+1.05**pp。後窗放寬到 9 微勝但前窗為負 → 雜訊。
(N07 另量到 fresh 1~7 的平均報酬兩窗排序不一致,池內大致同質,與此結論相符。)

**L 軸 loser_time_stop**(現行 15):**高原,收緊是災難、放寬在雜訊內**。
全窗 ΔCAGR / ΔSharpe:10 → −28.94 / −0.448;**15 = 0**;21 → −0.18 / −0.040;
26 → **+1.43** / −0.007;None → +1.42 / −0.009。
loser 26 是唯一三窗 CAGR 皆正者(+1.43 / +2.58 / +0.26pp)**但 Sharpe 兩窗為負、
MDD 由 −39.70% 惡化到 −42.28%** → 不過判準,不採用。放寬到 21 以上時 signal 出場
占比升到 97%(loser 閘實質消失),績效持平 → **15 是「剛好夠鬆」的位置,再鬆無收益、
再緊(10)立刻塌 −29pp**。

**本批結論**:S 的三個主控閥都已在最適點或高原上。**這是負結果,但也是對 S 的獨立
背書**——參數不是運氣站對位置,是每一軸都有可辨識的最適形狀(尖峰 / 平坦峰 / 高原)。
再要提升,必須靠**新資訊**(新因子)而不是重調現有旋鈕 → 接 N10。

## N10 — N08 過關因子的端到端對決(2026-08-07):七臂全敗,並暴露 N08 的方法缺陷

| 臂 | 全窗 ΔCAGR/ΔSharpe | 前窗 | 後窗 |
|---|---|---|---|
| H +hvn_dist | −1.64 / −0.066 | +1.63 / −0.024 | −4.81 / −0.119 |
| R +range_pos_60 | −1.61 / −0.030 | +2.48 / +0.040 | −5.56 / −0.112 |
| D +donchian_60 | −4.11 / −0.039 | −1.04 / +0.027 | −7.02 / −0.118 |
| HR +hvn+range | +1.69 / −0.010 | **+6.81 / +0.061** | **−3.34 / −0.094** |
| HRD 三個全加 | −7.24 / −0.143 | −3.32 / −0.073 | −10.88 / −0.229 |
| SWAP close_pos_20→range_pos_60 | −12.69 / −0.303 | −12.31 / −0.269 | −12.79 / −0.325 |

**全部在後窗為負**,無一過一致性。SWAP 兩窗皆 −12pp 以上 → **close_pos_20 明顯優於
range_pos_60**,F01 當年選它是對的。

**機制(事後很清楚)**:這三個候選全是**價格位置**軸,而現役六因子裡 `high_52w` 與
`close_pos_20` 已佔該軸兩席。加進去不是加資訊,是把價格位置從 2 席擴成 4 席,
從營收軸搶走權重。

**N08 的方法缺陷(本專案第二次踩同型錯)**:N08 量的是因子**單獨**的火箭鑑別力,
沒有控制「現有分數已經知道了多少」。與現有分數相關的因子在那種篩選下一樣會漂亮
——那是重複計數,不是增量。第一次踩是 F01 的「IC 顯著 ≠ 可交易」(靠端到端才發現),
這次同樣靠端到端才發現。**修法:N12 殘差條件篩選(先按分數分層、層內再看提升),
讓這類重複在篩選層就被擋掉,不必燒一輪回測。**

## N11 — 營收加速的自身標準化(2026-08-07):比原始因子更差,且指出標準化的邊界

把 `rev_yoy_accel` 除以該公司過去 12 次公布的加速離散度(z 分數),池內篩選與端到端皆敗:

| | 火箭提升 IS/OOS | 賠率 IS/OOS | 端到端全窗 ΔCAGR/ΔSharpe |
|---|---|---|---|
| rev_yoy_accel(現行) | 1.92 / 1.84 | 2.23 / 1.58 | — |
| accel_z | 1.72 / 1.52 | 2.04 / 1.62 | 取代 −18.71 / −0.255;加 0.5 −8.24 / −0.090 |

**教訓(與 N04 的 σ 標準化並列讀)**:**標準化不是萬靈丹,要看被標準化的量是什麼。**
對**風險門檻**(停損)標準化是對的——目的是讓每檔股票有相同的觸發機率;對**訊號強度**
標準化是錯的——營收加速的絕對幅度本身就是經濟意義(大幅加速 = 大事件),除以自身波動
等於把訊號本體丟掉,而且系統性地縮小了那些「營收本來就跳動劇烈」的微型股,
那正是 S 的火箭來源。

## N12 — 殘差條件篩選(2026-08-07):方法本身也不能當閘門,這是本輪最後的方法結論

修 N08 的缺陷:先按當日 canonical 分數分五層,**層內**再看因子的火箭提升——與現有分數
同軸者層內會塌回 ~1。方法自檢通過:現役 `close_pos_20`(權重 1.0)層內提升塌到
0.51/0.68(同號 0/5)、`high_52w` 塌到 0.96/1.08,確認方法抓得到重複計數。

層內賠率(火箭提升 ÷ 崩跌提升)排名:hvn_dist 1.60/1.63 → range_pos_60 1.49/1.53 →
rev_yoy 1.37/1.21 → sig20 1.36/1.30 → updays_20 1.16/1.07 → fvg_20 1.06/1.23。

**但這個排名沒有預測到 N10 的結果**:層內賠率最佳的 hvn_dist 與 range_pos_60,
端到端仍在後窗為負。**結論(本輪方法論的終點)**:

> **對 top-N 集中策略,任何池層級的篩選都不能當閘門。** 池層級量的是「五分位價差」,
> 而策略只交易分數分佈的極端尾巴(每日 424 檔候選裡只取 5 檔);加一個因子改變的是
> **哪 5 檔被選中**與**六因子權重如何重新分配**,這與五分位價差是不同的物件。
> 篩選只能**排序想法的優先順序**,唯一的裁決者是端到端回測。

順帶一個對 S 的側面背書:現役六因子裡 `mom_126_5`(權重 0.5)層內提升仍有 3.35/2.53、
`rev_seq` 2.07/1.49——**它們在自己已經參與的分數之內還保有鑑別力**,代表權重沒有給滿;
而 `close_pos_20`(權重 1.0)已經被榨乾。這與 R28 鄰域掃描收斂到現行權重相容。

## N13 — 候選名單深度 × 建倉節流(2026-08-07 預註冊)

**假設**:S 的 `_top_k = 5` 恰等於 `n_slots = 5`,**名單上沒有備選**。任一檔當日買不到
(漲停鎖死、無成交、現金不足),席位當天就空著,不會由第 6 名遞補。而 S 的候選正是
微型股營收火箭,漲停鎖死在它們身上是常態(引擎 E01 精準鎖死偵測即為此而生)。
N07 量到最賺的 10% 交易佔全部正報酬 54.1% —— **這種分佈下漏接一筆火箭的代價,
遠大於多接一筆平庸倉的代價**。本批動的是**執行完整度**,與前九批(出場、資金分配、
計分)全部不同軸。

**臂**:K 軸 top_k ∈ {5(現行), 6, 8, 10, 15};M 軸 max_new_per_day ∈ {1, 2(現行), 3, 5}。
兩軸各自單獨掃。**判準**同前:CAGR 與 Sharpe 同時不劣於現行 + 前後窗一致性。
**已知反向風險**:名單加深讓分數較低者有機會進場,若第 6-15 名品質明顯較差,
稀釋會蓋過補位收益——這正是要量的東西。

### N13 結果(2026-08-07):「漏接火箭」假設被乾淨證偽;節流 2 是尖銳最佳

**K 軸 top_k**(現行 5):全窗 ΔCAGR 6/8/10/15 = **−0.88pp(四者完全相同)**;
前窗 −1.74pp(四者相同);後窗 **0.00pp(四者相同)**。
→ **加深到 6 之後完全無變化**,代表「第 1-5 名買不到、由第 7 名以後遞補」的情形
**在全史裡幾乎不存在**。原假設(微型股漲停鎖死頻繁 → 常漏接)被證偽:
`max_new_per_day = 2` 的節流已經讓每日只需要 2 個可買標的,而前 6 名幾乎總有 2 檔可買。
加到 6 反而小輸(把第 6 名放進來 = 稀釋),故現行 5 為最適。

**M 軸 max_new_per_day**(現行 2):**尖銳內部最佳,三窗一致**。
全窗 ΔCAGR 1/3/5 = −11.07 / −10.79 / −29.81pp;前窗 −11.34 / −13.45 / −35.32pp;
後窗 −10.70 / −7.98 / −22.33pp。signal 出場占比隨節流放寬由 80.6% 掉到 58.2%
(建倉太快 → 倉位提前塞滿 → 更多倉被時間止損掃掉而非走完營收週期)。
→ R3 的「慢速建倉」經此確認為尖銳最佳,不是慣例。

**兩軸皆無提升。** 至此 S 的執行完整度軸也關閉。

## N14 — 出場錨點:事件錨 vs 進場錨(2026-08-07):本輪唯一的正面機制發現

**背景**:`rev_fresh_days` 是**日曆天**(assemble.py:163 `(date - avail).dt.total_days()`),
月營收 avail 固定為次月 10 日。故 S 的真實節奏是:**公布後 7 天內買進(10-17 日)→
抱到公布後第 26 天(次月 5 日)出場 → 恰好避開下一次公布**。晚進場者自動抱得短
(10 日進場抱 26 天、17 日進場抱 19 天),因為大家在同一天出場。

**要分離的混淆**:stale 26 勝出可能只是因為「三週」剛好是最佳持有長度,與「避開公布
事件」無關。本批把兩者拆開:T 軸關掉 stale(設 999)、改用進場後固定 N 個交易日出場,
讓每個人抱一樣長、出場日各自不同、必然有人抱過公布日。

| 臂 | 中位持有 | 全窗 ΔCAGR/ΔSharpe | 前窗 | 後窗 |
|---|---|---|---|---|
| E0 事件錨 stale26(現行) | 16 日 | — | — | — |
| T13 進場錨 time13 | 14 日 | −14.64 / −0.146 | −19.33 / −0.177 | −9.97 / −0.117 |
| **T16 進場錨 time16** | **17 日** | **−4.49 / −0.094** | **−5.13 / −0.091** | **−2.98 / −0.076** |
| T19 進場錨 time19 | 18 日 | −18.12 / −0.380 | −23.59 / −0.449 | −10.08 / −0.237 |
| T22 進場錨 time22 | 18 日 | −27.17 / −0.555 | −35.10 / −0.673 | −16.79 / −0.360 |

**結論(三窗一致)**:即使把持有長度調到與現行幾乎相同(T16 中位 17 日 vs E0 的 16 日),
事件錨仍勝 **3.0~5.1pp/年、Sharpe 勝 0.076~0.094**。
→ **S 的價值不只是「抱三週」,而是「在下一次營收公布前出場」這件事本身。**
這是本輪唯一的正面發現:它沒有提升績效(現行已是最佳),但**第一次用乾淨的對照組
量化證明了 S 的核心結構為什麼有效、以及它值多少**——先前只知道 stale 26 掃出來最好,
不知道那是持有長度的效果還是事件規避的效果。

**副產物**:S 的機制可以一句話寫下來——**「營收公布後一週內進場,抱過整個營收空窗期,
在下次公布前約 5 天出清」**。這句話先前不在任何文件裡。

## N15 — 個案診斷:S 為什麼選不到 2059 川湖(2026-08-08)

**問題來源**:使用者主觀交易川湖大賺,問 S 為何選不到。逐層歸因(`experiments/
n15_case_kingslide.py`),S 全史 679 筆交易中川湖 **0 筆**。三層疊加,每層都有數字:

1. **cfo 閘是主犯(95% 資格日被擋)**:全史月中資格日 775 天,cfo 閘擋掉 739 天,
   最終在池僅 36 天。**非資料缺漏**(cfo 欄 0 null)——川湖 cfo/ni 年中位 0.81~1.26
   屬健康值,但 S 池中位 1.09~1.49 更高;高成長期應收/存貨隨營收暴增壓低 CFO,
   2024(AI 行情最猛年)0.865 vs 池中位 1.377,整年被擋。唯一過閘的 2021
   (1.263 > 1.185)在池 33 天。
2. **排名差一名**:36 個在池日最佳名次第 6(2021-09-14),名次 ≤5 = 0 天。六因子
   百分位全在 70~92 但無單項爆表——S 的幾何連乘計分獎勵「單項 99 分的異常」
   (微型股 +300pp 加速),不獎勵「全能 85 分」。2024 之後只在池 1 天
   (2026-06-01,第 20 名,儘管 rev_yoy +172%)。
3. **節奏吃不到**:月中切片(11 日進、次月 5 日出)串接 2025 起 +207% vs 同錨買住
   +586%,**空手的公布窗貢獻 +123%**——川湖漲幅一大半在營收公布窗爆發,正是 S
   設計上空手的日子。N14 證明對 S 的獵物空手公布窗值 +3~5pp/年;川湖型正好鏡像。

**反事實(拿掉 cfo 閘)**:全窗 CAGR 84.17% → 65.77%(−18.40pp)、Sharpe 2.109 → 1.693、
MDD −39.7% → −46.6%;換到的川湖交易**只有 1 筆**(2026-07-15 進場,+17.3% 未平倉)
——前十一年連無閘版都排不進 top 5。**年賠 18.4pp 換一筆 +17%,閘不翻案。**
(B02 的 gate 分位 p50 內部最優亦維持。)

**結論**:S 選不到川湖是**物種分工**不是缺陷——cfo 閘、加速計分、避開公布窗三個
設計各自在 S 的獵場(微型股營收衝刺)被回測背書,而三者恰好逐項與「大型結構重估股」
相反。川湖在 `docs/leaders_by_domain.md` Tier 2A(Server rail 全球 #1)早被質化側
標出——抓這類股的系統是 Serenity 策展制,執行扳機在人工;使用者本次主觀進場即是
人工執行了該側邏輯。

## N16 — 慢速暴漲股普查(2026-08-08 預註冊;N15 川湖案的系統化)

**緣起**:使用者問「能不能由暴漲股反推一套大幅超越 S 的純量化策略」。此思路 =
F05(2026-07-10)已做過的 case-control:快速暴漲(60 日 +80%)前兆第一名是高已實現
波動(AUC 0.71),但**轉化雙敗**(前兆容器 CAGR 5% / MDD −69%:高波是雙尾放大器,
無條件買前兆 = 彩券盒)。**F05 的死角 = N15 揭露的慢速類**:川湖式「250 日 +200%」
結構重估從未被普查——F05 的 60 日窗根本抓不到它的形態。

**本批問三件事**(census only,不先做策略):
1. 慢速暴漲(fwd 250 交易日內最高收盤 ≥ +200%,eligible ADV≥5M/價≥10,per-code
   250d 冷卻)有多少?與快速暴漲(F05 定義)重疊多少?
2. 依進場日營收狀態四象限分類(全 eligible 截面百分位,門檻 70:
   HH 高年增高加速 / **HL 高年增低加速 = 川湖持續型** / LH 低年增高加速 = 轉機型 /
   LL 雙低 = 無基本面訊號),各類佔比與中位漲幅?
3. S 對每類的覆蓋漏斗:曾在池(≥1 日)→ 最佳名次 ≤5 → 實際交易;cfo 閘擋掉的比例?

**預測(先於執行)**:
- P1:HL(川湖持續型)佔慢速暴漲 ≥ 20%,且 S 實際交易覆蓋 < 10% —— 川湖非孤例。
- P2:LL(無基本面)為最大類(≥ 40%)—— 慢速版的 F05 結論:這一類在現有資料裡
  不可預測,需新資訊。
- P3:若 P1 成立 → N17 持續型容器(營收年增持續高檔池、無 cfo 閘、抱過公布窗的
  長持有)。**判準先寫死:3 年窗 CAGR ≥ 30% 且 MDD ≥ −40%(絕對門檻)才進正式驗證;
  定位是與 S 正交的第二本書。「大幅超越 S」由驗證說話,不由願望說話——base rate
  誠實披露:592 trials + F/G/R/N 線全部收斂於 S,資訊集掃空結論已兩度獨立成立。**

### N16 結果(2026-08-08):P1 部分證偽、P2 證實;N17 依預註冊不啟動

**census**:慢速(250 日 ≥+200%)974 起(2013-2025,2020 峰 244、2025 次峰 194)、
快速(60 日 ≥+80%)2,290 起。川湖自己 2 起:2023-02-07(+204%,**起漲時 LL 無基本面**)
、2024-08-05(+227%,HH)——連川湖都是「先漲、基本面後到」。

**四象限(慢速)**:LL 無基本面 **53.9%** / HH 19.0% / LH 12.1% / **HL 持續型 11.8%** /
NA 3.2%;四類中位漲幅同為 ~210-216%(類別不預測幅度)。S 覆蓋:曾在池(episode 窗內)
63-79%、最佳名次 ≤5 僅 24-29%、實際交易 10-15%——**漏斗最大損失在排名層**,此即
R20 神諭差距(oracle +3,900% vs S),G-line ML 已證現有特徵閉合不了。
**88.9% 慢速含快速爆發段**——慢速不是獨立物種,S 的事件機器看得見它們,選不中而已。

**預測裁決**:
- P1 **部分證偽**:HL 佔 11.8%(預測 ≥20%);S 覆蓋 10%(預測 <10%,壓線)。
  → **N17 持續型容器依預註冊不啟動**:~9 起/年的目標類 + F05c 稀釋失敗模式
  (持續池每日 ~180 檔,年基準率 ~5%,無判別器 = 彩券盒重演)。
- P2 **證實**:LL 53.9% ≥ 40%——暴漲股**多數**在起漲點無任何營收前兆,
  與 F05(快速類唯一前兆 = 高波動,雙尾對稱)合併 → 資料內不可預測。

**終論(與 F05/F-line/G-line 三方一致)**:「由果溯因」正是 S 的出身,此路在現有
資訊集內已兩度走盡(F05 快速類、N16 慢速類)。剩餘暴漲母體(LL 54%)的鑰匙全在
資訊集外:新聞/題材層(Serenity/Evergreen 領域)、真實揭露時點(revenue_first_seen
累積中)、TDCC 集保(僅 12 週,每日爬蟲自動累積,~2 年後可測)、日內(使用者閘)。
