...
Run(本機測):uv run --project . python -m quantlib.apex.refit --date 2026-07-20
"""
...
from quantlib.apex.experiments.m01_window_length import GRID, prep, run_config
from quantlib.apex.experiments.m02_refit_frequency import (BOOT_SEED, _metrics,
                                                           _rets, _sub_years)
from quantlib.apex.experiments.m03_refit_timing import cfg_name
...
_STATE = "src/quantlib/apex/state/last_refit_year.txt"
...
def maybe_run_annual(today: Date, email: bool = True) -> bool:
    ...
    print(body)
    if email:
        from quantlib.trading.live.notify import GmailNotifier
...
def main() -> None:
    ...
    args = ap.parse_args()
    from quantlib.trading.live.notify import today_taipei
