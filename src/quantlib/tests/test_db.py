...
def test_cache_connect_is_read_only_parallel() -> None:
    """Default quantlib.db.connect() must not take DuckDB's single-writer lock."""
    script = (
        "import os, sys; "
        f"sys.path.insert(0, {str((ROOT / 'src' / 'quantlib').resolve())!r}); "
    ...
    env = {**os.environ, "PYTHONPATH": str(ROOT / "src" / "quantlib")}
    procs = [
        subprocess.Popen(
