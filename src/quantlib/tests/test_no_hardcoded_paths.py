"""防復發法條:產物位置不得再寫成字面值,一律從 `quantlib.paths` 取。
...
- `src/quantlib/paths.py` 自己不算(它就是定義的地方)
- 逐字封存的歷史復原碼不算(`apex/rebuild/`、`apex/experiments/` 是
  transcript 原樣搶救回來的實驗紀錄,改動它們等於竄改研究史)

Run: uv run --project . python -m pytest src/quantlib/tests/test_no_hardcoded_paths.py
"""
...
#: 定義處與逐字封存的歷史紀錄
EXEMPT_FILES = {"src/quantlib/paths.py",
                "src/quantlib/tests/test_no_hardcoded_paths.py"}  # 定義處 + 法條本身
EXEMPT_DIRS = ("src/quantlib/apex/rebuild/", "src/quantlib/apex/experiments/")
    ...
    for p in sorted((REPO / "src" / "quantlib").rglob("*.py")):
        ...
        for lineno, s in _code_strings(tree):
            for bad in FORBIDDEN:
                if bad in s:
                    out.append(f"{rel}:{lineno} 硬編路徑 {s!r}(應改用 quantlib.paths)")
    return out
...
def test_no_hardcoded_product_paths() -> None:
    bad = _offenders()
    assert not bad, (
        f"發現 {len(bad)} 處硬編產物路徑——請改從 `quantlib.paths` 取:\n  "
        + "\n  ".join(bad[:20]))
...
def test_paths_module_is_never_shadowed() -> None:
    ...
    模組級加入 `from quantlib import paths` 後,Python 在編譯期就把整個函式裡的
