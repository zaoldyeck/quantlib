...
Run(單變體煙測): uv run --project . python -m quantlib.evergreen.ev26_engine
"""
...
from quantlib.apex.experiments.g01_ml_ranker import kpi
from quantlib.evergreen.harvest import build_feats, monthly_membership
...
class Lab:
    ...
    def __init__(self) -> None:
        ...
        self.regs = {n: pl.read_parquet(
            f"src/quantlib/evergreen/data/registry_{n}.parquet") for n in ["v1", "v2"]}
    ...
    def membership(self, reg_name: str, pool_months: int) -> pl.DataFrame:
        ...
        if key not in self._memb_cache:
            import quantlib.evergreen.harvest as hv
            old = hv.POOL_MONTHS
