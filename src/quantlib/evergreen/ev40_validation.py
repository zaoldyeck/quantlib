...
Run: uv run --project . python -m quantlib.evergreen.ev40_validation
依賴 cache: 是
"""
...
from quantlib.apex.validate import (block_bootstrap_cagr, deflated_sharpe,
                                    pbo_cscv)
from quantlib.evergreen.ev36_walkforward import C, seg_kpi
from quantlib.evergreen.ev38_exhaust import FOLDS, LabX, run
...
def permutation(lab: LabX, real: float, n: int = 100) -> float:
    ...
    from quantlib.apex import data as adata
    con = adata.connect()
...
def main() -> None:
    ...
    resdf = (pl.read_parquet("src/quantlib/evergreen/data/ev38_results.parquet")
