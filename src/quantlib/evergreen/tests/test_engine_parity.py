...
的降級版。根治 = 生產路徑一律呼叫 quantlib.evergreen.engine;本測試鎖死
「engine.replay_nav 重現 live_config.json 記錄的 train CAGR/MDD」——任何未來
對引擎資料層/計分/simulate 接線的漂移,都會讓本測試變紅。

Run: uv run --project . python -m quantlib.evergreen.tests.test_engine_parity
     或 uv run --project . pytest src/quantlib/evergreen/tests/test_engine_parity.py
...
from quantlib.evergreen.engine import LIVE_CONFIG, EvergreenData, replay_nav
from quantlib.evergreen.ev36_walkforward import seg_kpi
from quantlib import testkit
