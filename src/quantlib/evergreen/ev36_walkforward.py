...
Run: uv run --project . python -m quantlib.evergreen.ev36_walkforward
...
OUT_RES = "src/quantlib/evergreen/data/ev36_results.parquet"
OUT_NAV = "src/quantlib/evergreen/data/ev36_top1_nav.parquet"


def load_registry() -> pl.DataFrame:
    reg = pl.read_parquet("src/quantlib/evergreen/data/registry_v3.parquet")
    pilot = (pl.read_parquet("src/quantlib/evergreen/data/ev28_pilot_labels.parquet")
...
def bench_same_window() -> dict:
    s = pl.read_parquet("src/quantlib/apex/ledger/curves/T0334.parquet")
