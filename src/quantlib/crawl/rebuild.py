...
Run:
  uv run --project . python -m quantlib.crawl.rebuild --source daily_trading_details
  uv run --project . python -m quantlib.crawl.rebuild --all
  uv run --project . python -m quantlib.crawl.rebuild --source daily_quote --dry-run  # 只解析算列數不寫
"""
...
def rebuild_operating_revenue() -> int:
    ...
    import glob
    from quantlib.crawl.sources import operating_revenue as opr
...
def rebuild_ex_right_dividend() -> int:
    ...
    import glob
    from quantlib.crawl.sources import ex_right_dividend as exd
    frames = []
...
def rebuild_capital_reduction() -> int:
    import glob
    from quantlib.crawl.sources import capital_reduction as cr
    frames = []
...
def rebuild_taifex_daily() -> int:
    ...
    import glob
    from quantlib.crawl.sources import taifex
...
def rebuild_insider_holding() -> int:
    ...
    from quantlib.crawl.sources import insider_holding as ins
    frames = []
...
def rebuild_treasury_buyback() -> int:
    import glob
    from quantlib.crawl.sources import treasury_stock_buyback as tsb
    frames = []
...
def main() -> None:
    ...
    if args.quarterly:
        for name, fn in QUARTERLY_REBUILDS.items():
            fn()
        print("[rebuild] 季頻/特殊源完成;財報鏈另跑 quantlib.crawl.rebuild_financials")
        return
