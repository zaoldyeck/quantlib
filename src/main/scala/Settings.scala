...
object Settings {
  ...
  case class FinancialAnalysisSetting(year: Int = LocalDate.now.getYear) extends Setting {
    ...
    val markets: Seq[Detail] = year match {
      ...
      case y if y > 2011 && y < 2015 => Seq(new TwseBeforeIFRSsDetail, new TwseAfterIFRSsDetail, new TpexBeforeIFRSsDetail, new TpexAfterIFRSsDetail)
  ...
  case class MarginTransactionsSetting(date: LocalDate = LocalDate.now) extends Setting {
    val twse: TwseDetail = new TwseDetail(LocalDate.of(2001, 1, 1), None, date) {
