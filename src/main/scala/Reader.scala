...
class Reader {
      ...
      file =>
        ...
        val dbIOActions = rows.map {
          values =>
            ...
            val data = Query((year :: companyCode :: values(1) :: transferValues.head :: transferValues(1) ::
  ...
  def readOperatingRevenue(): Unit = {
    operatingRevenue.dir.toDirectory.files.foreach {
      file =>
        ...
        val dbIOActions = rows.map {
          values =>
            ...
            val data = Query((y, m, Option(values(4)), companyCode, values(3), transferValues.head, transferValues(1), transferValues(2), transferValues(3), transferValues(4), transferValues(5), transferValues(6), transferValues(7)))
  ...
  def readIndex(): Unit = {
    index.dir.toDirectory.files.foreach {
      file =>
        val reader = QuantlibCSVReader.open(file.jfile, "Big5")
        val fileNamePattern = """(\d+)_(\d+)_(\d+).csv""".r
        val fileNamePattern(year, month, day) = file.name
  ...
  def readExRightDividend(): Unit = {
    exRightDividend.dir.toDirectory.files.foreach {
      file =>
        val reader = QuantlibCSVReader.open(file.jfile, "Big5")
        ...
        val dbIOActions = rows.map {
          values =>
            ...
            val datePattern(year, month, day) = values.head
