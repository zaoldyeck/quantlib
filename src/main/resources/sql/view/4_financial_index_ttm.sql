...
--    (2026-07-23 稽核 D-financial-ratios 已修正本檔學理 bug:TTM 總資產週轉率 lag5→lag4、
--     現金比率分母改流動負債、ROIC 分子改營業利益 net_operating_income、
--     負權益/負資本運用額 rank guard、每股 FCF 除股數〔股本/10〕;
--     view 仍 deprecated,Python raw_quarterly.py 為唯一真源。)
-- ============================================================================
create view financial_index_ttm as
with industry as (select distinct on (company_code) company_code, industry
     ...
     financial_index as (select cfswt.market,
                                ...
                                company_name,
                                industry,
                                sum(net_operating_income)
                                over (partition by cfswt.company_code order by cfswt.year, cfswt.quarter rows between 3 preceding and current row) /
                                nullif(greatest(total_assets - total_current_liabilities, 0), 0)                                                              as roic,
                                sum(profit)
                                over (partition by cfswt.company_code order by cfswt.year, cfswt.quarter rows between 3 preceding and current row) /
                                nullif((total_assets + lag(total_assets, 4)
                                ...
                                (receivable + lag(receivable, 4)
                                              over (partition by cfswt.company_code order by cfswt.year, cfswt.quarter)) /
                                2 * 365 /
                                nullif(sum(total_operating_revenue)
                                       over (partition by cfswt.company_code order by cfswt.year, cfswt.quarter rows between 3 preceding and current row),
                                       0)                                                                                                          as days_sales_outstanding,
                                sum(ocf)
                                over (partition by cfswt.company_code order by cfswt.year, cfswt.quarter rows between 3 preceding and current row) as ocf,
                                sum(profit)
                                over (partition by cfswt.company_code order by cfswt.year, cfswt.quarter rows between 3 preceding and current row) as profit,
                                total_non_current_liabilities,
                                total_capital_stock,
                                (sum(total_operating_revenue)
                                 over (partition by cfswt.company_code order by cfswt.year, cfswt.quarter rows between 3 preceding and current row) -
                                 sum(total_operating_costs)
                                 over (partition by cfswt.company_code order by cfswt.year, cfswt.quarter rows between 3 preceding and current row)) /
                                nullif(sum(total_operating_revenue)
                                       over (partition by cfswt.company_code order by cfswt.year, cfswt.quarter rows between 3 preceding and current row),
                                       0)                                                                                                          as gross_margin,
                                ...
                                nullif(((total_assets + lag(total_assets, 4)
                                                        over (partition by cfswt.company_code order by cfswt.year, cfswt.quarter)) /
                                        2),
                                       0)                                                                                                          as total_assets_turnover,
                                sum(total_operating_revenue)
                                over (partition by cfswt.company_code order by cfswt.year, cfswt.quarter rows between 3 preceding and current row) as total_operating_revenue,
                                sum(profit)
                                over (partition by cfswt.company_code order by cfswt.year, cfswt.quarter rows between 3 preceding and current row) /
                                nullif(sum(total_operating_revenue)
                                       over (partition by cfswt.company_code order by cfswt.year, cfswt.quarter rows between 3 preceding and current row),
                                ...
                                (inventories + lag(inventories, 4)
                                               over (partition by cfswt.company_code order by cfswt.year, cfswt.quarter)) /
                                2 * 365 /
                                nullif(sum(total_operating_costs)
                                       over (partition by cfswt.company_code order by cfswt.year, cfswt.quarter rows between 3 preceding and current row),
                                       0)                                                                                                          as days_sales_of_inventory,
                                inventories / total_assets                                                                                         as inventories_ratio,
                                receivable / total_assets                                                                                          as receivables_ratio,
                                sum(eps)
                                over (partition by cfswt.company_code order by cfswt.year, cfswt.quarter rows between 3 preceding and current row) as eps,
                                (sum(ocf)
                                 over (partition by cfswt.company_code order by cfswt.year, cfswt.quarter rows between 3 preceding and current row) +
                                 sum(capital_expense)
                                 over (partition by cfswt.company_code order by cfswt.year, cfswt.quarter rows between 3 preceding and current row)) /
                                nullif(total_capital_stock / 10.0, 0)                                                                                     as fcf_per_share
